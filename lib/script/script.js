/*!
 * script.js - script interpreter for hsk
...
class Script {
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKOUTPUTVERIFY: {
          ...
          const version = stack.getInt(-2, minimal, 4);
          const hash = stack.get(-1);

          const res = checkOutput(tx, index, value, version, hash);
  ...
  /**
   * Get the blake2b of the raw script.
   * @param {String?} enc
   * @returns {Hash}
   */

  blake256(enc) {
    let hash = blake2b.digest(this.toRaw());
    if (enc === 'hex')
  ...
  static verify(witness, addr, tx, index, value, flags) {
    ...
    if (addr.version === 0) {
      if (addr.hash.length === 32) {
        ...
        if (!blake2b.digest(witnessScript).equals(addr.hash))
