...
class Script {
  ...
  /**
   * Get the hash256 of the raw script.
   * @param {String?} enc
   * @returns {Hash}
   */

  hash256(enc) {
    let hash = hash256.digest(this.toRaw());
    if (enc === 'hex')
  ...
  static verify(witness, addr, tx, index, value, flags) {
    ...
    if (addr.version === 0) {
      if (addr.hash.length === 32) {
        ...
        if (!hash256.digest(witnessScript).equals(addr.hash))
...
function checksig(msg, sig, key) {
  return secp256k1.verifyRS(msg, sig.slice(0, -1), key);
}
