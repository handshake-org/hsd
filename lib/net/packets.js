...
class VersionPacket extends Packet {
  ...
  toWriter(bw) {
    ...
    this.remote.toWriter(bw);
    this.local.toWriter(bw);
  ...
  fromReader(br) {
    ...
    this.remote.fromReader(br);
    this.local.fromReader(br);
...
class AddrPacket extends Packet {
  ...
  fromRaw(data) {
    ...
    for (let i = 0; i < count; i++)
      this.items.push(NetAddress.fromReader(br));
...
class GetProofPacket extends Packet {
  ...
  /**
   * Instantiate getproof packet from buffer reader.
   * @param {BufferReader} br
   * @returns {AuthProposePacket}
   */
  ...
  /**
   * Instantiate getproof packet from serialized data.
   * @param {Buffer} data
   * @param {String?} enc
   * @returns {AuthProposePacket}
   */
...
class ProofPacket extends Packet {
   ...
   * @constructor
   * @param {Number} version
   * @param {TrieProof} proof
   */

  constructor(version, proof) {
    super();

    this.cmd = 'proof';
    this.type = exports.types.PROOF;
    this.version = version || 0;
    this.proof = proof || new TrieProof();
  }
  ...
  toWriter(bw) {
    bw.writeU8(this.version);
    this.proof.toWriter(bw);
    return bw;
  }

  /**
   * Serialize proof packet.
   * @returns {Buffer}
   */
  ...
  /**
   * Instantiate proof packet from buffer reader.
   * @param {BufferReader} br
   * @returns {AuthProposePacket}
   */
  ...
  /**
   * Instantiate proof packet from serialized data.
   * @param {Buffer} data
   * @param {String?} enc
   * @returns {AuthProposePacket}
   */
