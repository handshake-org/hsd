...
const {Proof} = require('btrie/research/merklix/proof');
...
const ZERO_HASH160 = Buffer.alloc(0x00, 20);
...
class RejectPacket extends Packet {
  ...
  read(br) {
    ...
    this.code = br.readU8();
    this.reason = br.readString('ascii', br.readU8());
...
class ProofPacket extends Packet {
  ...
  getSize() {
    let size = 52;
    size += this.proof.getSize(blake2b, 160);
    return size;
  }
  ...
  write(bw) {
    bw.writeHash(this.root);
    bw.writeBytes(this.key);
    this.proof.writeBW(bw, blake2b, 160);
    return this;
  }
  ...
  read(br) {
    this.root = br.readHash();
    this.key = br.readBytes(20);
    this.proof = Proof.readBR(br, blake2b, 160);
    return this;
  }
}
...
exports.fromRaw = function fromRaw(type, data) {
  switch (type) {
    case types.VERSION:
    ...
    default:
      return UnknownPacket.fromRaw(data, type);
  }
};
