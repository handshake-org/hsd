...
const {BrontideSocket} = require('./brontide');
...
class Peer extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.id = -1;
    this.socket = null;
    this.brontide = new BrontideSocket();
  ...
  sendProof(root, key, nodes) {
    this.logger.info(
      'Sending proof (%s).',
      this.hostname());

    this.send(new packets.ProofPacket(root, key, nodes));
  }
