...
class Peer extends EventEmitter {
  ...
  increaseDos(dosScore, time) {
    ...
    // 30mins, i.e. 1800s.
    const seconds = (time - this.lastDos) / 1000;
    if (seconds > common.DOS_RESET_TIME) {
      this.dosScore = 0;
    } else if (this.dosScore > 1 && seconds > 0) {
      if (seconds < 64)
        this.dosScore *= common.DOS_DECAY_FACTOR[seconds | 0];
      else
        this.dosScore *= Math.exp(-1.0 * seconds * (Math.log(2) / 60));
    }
    this.lastDos = time;
...
class PeerOptions {
  ...
  constructor(options) {
    ...
    this.dosScore = common.BAN_SCORE;
