...
class Peer extends EventEmitter {
  ...
  _bind(socket) {
    ...
    else if (this.isBrontide())
      this.channel = this.brontide;

    assert(this.channel);

    this.brontide.once('error', (err) => {
      this.error(err);
      this.destroy();
    });
  ...
  accept(socket) {
    ...
    switch (this.type) {
      case this.options.peerTypes.STANDARD:
        this.connected = true;
        this.time = Date.now();
        break;
      case this.options.peerTypes.BRONTIDE:
        this.brontide.accept(socket, this.identityKey);
        break;
      default:
        this.error('Unkown peer type.');
        break;
    }
  ...
  initConnect() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.channel.once('connect', () => {
        ...
        if (this.isBrontide() && !this.outbound)
  ...
  write(data) {
    ...
    if (this.channel.write(data) === false)
  ...
  peerType() {
    return this.options.getPeerType(this.type);
  }

  /**
   * Checks if the peer type is standard.
   * @return {Boolean}
   */

  isStandard() {
    return this.type === this.options.peerTypes.STANDARD;
  }

  /**
   * Checks if the peer type is brontide.
   * @return {Boolean}
   */

  isBrontide() {
    return this.type === this.options.peerTypes.BRONTIDE;
  }
...
class PeerOptions {
  ...
  constructor(options) {
    ...
    this.nonces = null;
    this.peerTypes = null;
    this.peerTypesByVal = null;
    this.type = null;
    this.getTypeByAddr = PeerOptions.getTypeByAddr;
    this.getPeerType = PeerOptions.getPeerType;
  ...
  fromOptions(options) {
    ...
    if (options.nonces != null) {
      assert(typeof options.nonces === 'object');
      assert(typeof options.nonces.alloc === 'function');
      assert(typeof options.nonces.has === 'function');
      assert(typeof options.nonces.remove === 'function');
      this.nonces = options.nonces;
    }

    if (options.peerTypes != null) {
      assert(typeof options.peerTypes === 'object');
      this.peerTypes = options.peerTypes;
    }

    if (options.peerTypesByVal != null) {
      assert(typeof options.peerTypesByVal === 'object');
      this.peerTypesByVal = options.peerTypesByVal;
    }
    ...
    if (options.getTypeByAddr != null) {
      assert(typeof options.getTypeByAddr === 'function');
      this.getTypeByAddr = options.getTypeByAddr;
    }

    if (options.getPeerType != null) {
      assert(typeof options.getPeerType === 'function');
      this.getPeerType = options.getPeerType;
    }
  ...
  /**
   * Get the peer type by address.
   * See PoolOptions.peerTypes.
   * @private
   * @param {NetAddress}
   * @returns {Number}
   */

  static getTypeByAddr(addr) {
    return 0;
  }
