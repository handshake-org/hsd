...
class Peer extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.halflife = (0.693 / (this.options.p2prps / this.options.dosScore));
  ...
  write(data) {
    ...
    if (this.brontide.write(data) === false)
   ...
   * @param {Number} banScore
   * @returns {Boolean}
   */

  increaseBan(banScore) {
  ...
  /**
   * Increase dosscore on peer.
   * See: https://github.com/btcsuite/btcd/blob/master/connmgr/dynamicbanscore.go
   * @param {Number} dosScore
   * @param {Number} time
   * @returns {Boolean}
   */

  increaseDos(dosScore, time) {
    // dosScore is a time-bound transient banscore useful to detect DOS
    // attacks by repeated requests for a message (e.g. getproof).  Calling
    // increaseBan with a dosScore multiple times will increase dosScore in a
    // logarithmically decaying fashion wrt time between requests to penalize
    // short bursts of repeated requests.  The dosScore decays back to 0 in
    // 30s.
    const seconds = ((time - this.lastDos) / 1000) | 0;
    if (seconds > common.DOS_RESET_TIME) {
      this.dosScore = 0;
    } else {
        this.dosScore *= Math.exp(-1.0 * this.halflife * seconds * Math.log(2));
    }
    this.lastDos = time;

    this.dosScore = (this.dosScore | 0) + dosScore;

    if (this.dosScore >= this.options.dosScore) {
      this.logger.debug('DOS threshold exceeded (%s).', this.hostname());
      this.ban();
      return true;
    }
...
class PeerOptions {
  ...
  constructor(options) {
    ...
    this.dosScore = common.DOS_SCORE;
    this.p2prps = common.P2P_RPS;
  ...
  fromOptions(options) {
    ...
    if (options.dosScore != null) {
      assert(typeof options.dosScore === 'number');
      this.dosScore = options.dosScore;
    }

    if (options.p2prps != null) {
      assert(typeof options.p2prps === 'number');
      this.p2prps = options.p2prps;
    }
