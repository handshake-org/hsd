...
class HostList {
   ...
   * of standard NetAddresses. Only
   * addresses without keys are included.
   *
   * @returns {NetAddress[]}
   */
  ...
  addSeed(host) {
    let ip = IP.fromHostname(host, this.network.port);
    if (ip.key)
      ip = IP.fromHostname(host, this.network.brontidePort);
  ...
  removeNode(host) {
    ...
    if (addr.key)
      addr = IP.fromHostname(host, this.network.brontidePort);
...
class HostEntry {
  ...
  toJSON() {
    return {
      addr: this.addr.fullname(),
      src: this.src.fullname(),
