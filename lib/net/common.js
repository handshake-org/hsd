...
/**
 * Amount of time after which to reset DOS peers.
 * @const {Number}
 * @default
 */

exports.DOS_RESET_TIME = 30;

/*
 * Precomputed decay factor for first 1024 seconds.
 * The decay is logarithmic with respect to time.
 * See: http://mathworld.wolfram.com/ExponentialDecay.html
 * N(t) = N0 * (e ^ (-lambda * t))
 * @const {Object}
 * @default
 */

const dosDecayFactor = {};
for (let i=0; i < 1024; i++)
    dosDecayFactor[i] = Math.exp(-1.0 * i * Math.log(2));

exports.DOS_DECAY_FACTOR = dosDecayFactor;
...
/**
 * DOS score threshold before ban is placed in effect.
 * @const {Number}
 * @default
 */

exports.DOS_SCORE = 144;

/**
 * Requests per second for P2P messages. Breaching this will constitute a DOS
 * ban. Currently only used for getproof.
 * @const {Number}
 * @default
 */
