...
class Pool extends EventEmitter {
  ...
  async loadKey() {
    if (!this.options.prefix || this.options.memory)
      return;

    if (fs.unsupported)
      return;

    const filename = path.resolve(this.options.prefix, 'key');

    let data = null;

    if (!this.options.newKey) {
      try {
    ...
    if (!data) {
      ...
      await fs.writeFile(filename, key.toString('hex') + '\n');
      return;
    }
    ...
    if (key.length !== 32 || !secp256k1.privateKeyVerify(key))
      throw new Error('Invalid identity key file.');

    this.options.identityKey = key;
    this.hosts.address.setKey(secp256k1.publicKeyCreate(key));
  }
  ...
  async handleCmpctBlock(peer, packet) {
    ...
    this.logger.debug(
      ...
      block.hash('hex'), block.count, block.totalTX, peer.hostname());
  ...
  async handleProof(peer, packet) {
    const {root, key, nodes} = packet;

    const [code, data] = proof.verify(root, key, nodes);
    ...
    if (!data) {
      this.emit('proof', key, null);
      return;
    }

    if (data.length > 512) {
      this.logger.warning(
        'Peer sent us an invalid data length: %s/%s/%d (%s)!',
        key.toString('hex'),
        root.toString('hex'),
        code,
        peer.hostname());
  ...
  hasTX(hash) {
    ...
    if (!this.mempool) {
      ...
      if (this.mempool.hasReject(hash)) {
        this.logger.spam('Saw known reject of %s.', hash);
        return true;
      }
    }
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    if (this.network.identityKey)
      this.identityKey = this.network.identityKey;
