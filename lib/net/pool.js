...
const peerTypes = Peer.peerTypes;
...
class Pool extends EventEmitter {
  ...
  init() {
    ...
    this.server.on('connection', (socket) => {
      this.handleSocket(socket, peerTypes.STANDARD);
      this.emit('connection', socket);
    });

    this.brontide.on('connection', (socket) => {
      this.handleSocket(socket, peerTypes.BRONTIDE);
      this.emit('connection', socket);
    });
   ...
   * @param {Number} type - see Peer.peerTypes
   */
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    if (options.maxOutbound != null) {
      ...
      assert((options.maxOutbound >>> 0) === options.maxOutbound);
    ...
    if (options.brontideMaxOutbound != null) {
      ...
      assert((options.brontideMaxOutbound >>> 0)
        === options.brontideMaxOutbound);
