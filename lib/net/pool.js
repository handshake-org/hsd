...
class Pool extends EventEmitter {
  ...
  async handleGetProof(peer, packet) {
    ...
    const snapshot = this.cdb.trie.snapshot(root);

    const nodes = await snapshot.prove(key);
    const proof = new TrieProof(height, root, key, nodes);
