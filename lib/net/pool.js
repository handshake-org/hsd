...
class Pool extends EventEmitter {
  ...
  async _handleTX(peer, packet) {
    ...
    if (block) {
      const whash = tx.witnessHash('hex');
  ...
  async handleGetProof(peer, packet) {
    const {version, key} = packet;

    if (version !== 0) {
      this.logger.warning(
        'Ignoring unknown proof request: %d (%s).',
        version,
        peer.hostname());
      return;
    }
    ...
    // XXX What key format?
    // TODO: Use a decaying ban score for this call!
    // Add option to disable (opt-in disabling)
    const nodes = await this.chain.cdb.prove(root, key);
    const data = await this.chain.cdb.getData(key, height);

    if (!data) {
      this.logger.warning(
        'Ignoring non-provable key: %s (%s).',
        key.toString('hex'),
        peer.hostname());
      return;
    }
  ...
  async handleProof(peer, packet) {
    const {version, proof} = packet;
    const {height, root, key, data} = proof;

    if (version !== 0) {
      this.logger.warning(
        'Peer send an invalid version: %d (%s).',
        version,
        peer.hostname());
      peer.increaseBan(100);
      return;
    ...
    if (data.length > 1024) {
      this.logger.warning(
        'Peer send invalid data length: %d (%s).',
        data.length,
        peer.hostname());
      peer.increaseBan(100);
      return;
    ...
    if (height > this.chain.height) {
      this.logger.warning(
        'Peer sent us a proof for a block we do not have: %d (%s).',
        height,
        peer.hostname());
      peer.increaseBan(5);
      return;
    ...
    if (height < this.chain.height - 48) {
      this.logger.warning(
        'Peer sent us a very old proof: %d (%s).',
        height,
        peer.hostname());
      peer.increaseBan(20);
      return;
    ...
    if (entry.trieRoot !== root.toString('hex')) {
      this.logger.warning(
        'Peer may be on an alternate chain: %d/%s (%s).',
        height,
        root.toString('hex'),
        peer.hostname());
      peer.increaseBan(10);
    ...
    const [code, hash] = proof.verify();

    if (code !== 0) {
      this.logger.warning(
        'Peer sent us an invalid proof: %s/%s/%d (%s)!',
        key.toString('hex'),
        root.toString('hex'),
        code,
        peer.hostname());
    ...
    if (hash.length !== 32) {
      this.logger.warning(
        'Peer sent us an invalid hash length: %s/%s/%d (%s)!',
        key.toString('hex'),
        root.toString('hex'),
        code,
        peer.hostname());
    ...
    if (!blake2b.digest(data).equals(hash)) {
      this.logger.warning(
        'Peer sent us an invalid record: %s/%s/%d (%s)!',
        key.toString('hex'),
        root.toString('hex'),
        code,
        peer.hostname());
