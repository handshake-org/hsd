...
const PeerOptions = Peer.PeerOptions;
...
class Pool extends EventEmitter {
  ...
  init() {
    ...
    this.server.on('connection', (socket) => {
      this.handleSocket(socket, this.options.peerTypes.STANDARD);
      this.emit('connection', socket);
    });

    this.brontide.on('connection', (socket) => {
      this.handleSocket(socket, this.options.peerTypes.BRONTIDE);
      this.emit('connection', socket);
    });
    ...
    this.brontide.on('listening', () => {
      ...
      this.logger.info(
        'Pool brontide server listening on %s (port=%d).',
        data.address, data.port);
      this.emit('listening', data);
    });
  ...
  async open() {
    ...
    this.logger.info('Brontide Pool loaded (maxpeers=%d).',
      this.options.brontideMaxOutbound);

    this.logger.info('Brontide Pool identity key: %s.',
      base32.encode(this.hosts.address.key));
  ...
  async listen() {
    ...
    const {port, host, brontidePort, brontideHost} = this.options;

    this.server.maxConnections = this.options.maxInbound;
    await this.server.listen(port, host);

    this.brontide.maxConnections = this.options.brontideMaxInbound;
    await this.brontide.listen(brontidePort, brontideHost);
  }
  ...
  getKey(enc) {
    if (enc === 'base32')
      return base32.encode(this.hosts.address.key);
    if (enc === 'hex')
      return this.hosts.address.key.toString('hex');
    return this.hosts.address.key;
  }
   ...
   * @param {Number} type - see PoolOptions.peerTypes
   */
  ...
  handleSocket(socket, type) {
    ...
    const {brontideMaxInbound, maxInbound} = this.options;
    if (type === this.options.peerTypes.BRONTIDE
        && this.peers.brontide.inbound >= brontideMaxInbound) {
      this.logger.debug('Ignoring peer: too many brontide inbound (%s).', ip);
      socket.destroy();
      return;
    }

    if (type === this.options.peerTypes.STANDARD
        && this.peers.standard.inbound >= maxInbound) {
      this.logger.debug('Ignoring peer: too many standard inbound (%s).', ip);
      socket.destroy();
      return;
    }
  ...
  addLoader() {
    ...
    // There are no outbound peers. Create one.
    const addr = this.getHost();

    if (!addr)
      return;

    // Check that the NetAddress has a key
  ...
  createOutbound(addr) {
    ...
    options.type = this.options.getTypeByAddr(addr);
  ...
  createInbound(socket, type) {
    assert(typeof type === 'number');
    const options = new PeerOptions(this.options);
    options.type = type;

    const peer = Peer.fromInbound(options, socket);
  ...
  async handleGetAddr(peer, packet) {
    ...
    const addrs = this.hosts.toStandardArray();
    const items = [];
  ...
  async handleAddr(peer, packet) {
    ...
    for (const addr of addrs) {
      ...
      // Ignore keys sent over the network to prevent
      // spamming of fake keys.
      if (addr.hasKey())
        continue;
  ...
  addInbound(socket, type) {
    if (!this.opened) {
      socket.destroy();
      return;
    }

    const peer = this.createInbound(socket, type);
   ...
   * Argument specifies preferred type,
   * see PoolOptions.peerTypes. When not
   * specified, can return any type.
   * @param {Number} type
   * @returns {NetAddress}
   */
  ...
  addOutbound(type) {
    ...
    if (type == null) {
      if (needBrontideOutbound && random(10) < 2)
        type = this.options.peerTypes.BRONTIDE;
      else
        type = this.options.peerTypes.STANDARD;
    }
...
class PoolOptions {
  ...
  constructor(options) {
    ...
    this.brontideHost = '0.0.0.0';
    this.brontidePort = this.network.brontidePort;
    this.brontidePublicHost = '0.0.0.0';
    this.brontidePublicPort = this.network.brontidePort;
    this.maxOutbound = 8;
    this.maxInbound = 8;
    this.brontideMaxInbound = 2;
    this.brontideMaxOutbound = 0;
    ...
    this.peerTypesByVal = {
      [this.peerTypes.STANDARD]: 'STANDARD',
      [this.peerTypes.BRONTIDE]: 'BRONTIDE'
    };
  ...
  fromOptions(options) {
    ...
    if (options.brontideHost != null) {
      assert(typeof options.host === 'string');
      const raw = IP.toBuffer(options.brontideHost);
      this.brontideHost = IP.toString(raw);
      if (IP.isRoutable(raw))
        this.brontidePublicHost = this.host;
    }
    ...
    if (options.brontidePort != null) {
      ...
      this.brontidePublicPort = options.brontidePort;
    }

    if (options.brontidePublicHost != null) {
      assert(typeof options.brontidePublicHost === 'string');
      this.brontidePublicHost = IP.normalize(options.brontidePublicHost);
    }

    if (options.brontidePublicPort != null) {
      assert((options.brontidePublicPort & 0xffff)
        === options.brontidePublicPort);
      this.brontidePublicPort = options.brontidePublicPort;
    }
    ...
    if (options.brontideMaxInbound != null) {
      assert(typeof options.brontideMaxInbound === 'number');
      this.brontideMaxInbound = options.brontideMaxInbound;
    }

    if (options.brontideMaxOutbound != null) {
      assert(typeof options.brontideMaxOutbound === 'number');
      this.brontideMaxOutbound = options.brontideMaxOutbound;
    }
    ...
    if (options.only != null) {
      ...
      if (options.only.length > 0) {
        ...
        const [standard, brontide] = this.countHostnames(this.nodes);
        this.maxOutbound = standard;
        this.brontideMaxOutbound = brontide;
      }
    }
  ...
  /**
   * Count the number of hosts with
   * keys and without keys.
   * @param {Array}
   * @returns {Number[]}
   */

  countHostnames(hosts) {
  ...
  getTypeByAddr(addr) {
    if (addr.hasKey())
      return this.peerTypes.BRONTIDE;
    return this.peerTypes.STANDARD;
  }

  /**
   * Get the human readable peer type.
   * @returns {String}
   */

  getPeerType(type) {
    return this.peerTypesByVal[type];
  }
...
class PeerList {
  ...
  add(peer) {
    ...
    let container = this.standard;
    if (peer.isBrontide())
      container = this.brontide;

    if (peer.outbound)
      container.outbound += 1;
    else
      container.inbound += 1;
  }
