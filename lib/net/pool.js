...
const proofs = require('urkel/lib/proof');
...
class Pool extends EventEmitter {
  ...
  async handleClaim(peer, packet) {
    const hash = packet.claim.hash('hex');
    const unlock = await this.locker.lock(hash);
    try {
  ...
  async _handleClaim(peer, packet) {
    ...
    if (!this.resolveClaim(peer, hash)) {
      this.logger.warning(
        'Peer sent unrequested claim: %s (%s).',
        claim.hash('hex'), peer.hostname());
      peer.destroy();
      return;
    }
  ...
  async handleProof(peer, packet) {
    ...
    if (proof.value && proof.value.length > 512) {
      this.logger.warning(
        'Peer sent us an invalid data length: %s/%s (%s)!',
        key.toString('hex'),
        root.toString('hex'),
        peer.hostname());
      peer.increaseBan(100);
    ...
    const [code, data] = proofs.verify(blake2b, 256, root, key, proof);
...
class BroadcastItem extends EventEmitter {
  ...
  inspect() {
    ...
    return `<BroadcastItem: type=${name} hash=${hash}>`;
  }
}
