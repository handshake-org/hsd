...
class NetAddress {
  ...
  fromOptions(options) {
    ...
    if (options.pubkey) {
      assert(Buffer.isBuffer(options.pubkey));
  ...
  fromJSON(json) {
    ...
    assert(typeof json.pubkey === 'string');
    ...
    this.time = json.time;
    this.hostname = IP.toHostname(this.host, this.port);
    this.pubkey = Buffer.from(json.pubkey, 'hex');
    return this;
  }
