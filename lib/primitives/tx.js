...
class TX {
  ...
  hash(enc) {
    ...
    if (!h) {
      h = blake2b.digest(this.toNormal());
      if (!this.mutable)
        this._hash = h;
    }
  ...
  witnessHash(enc) {
    ...
    if (!h) {
      h = blake2b.digest(this.toRaw());
      if (!this.mutable)
        this._whash = h;
    }

    if (enc === 'hex') {
      let hex = this._whhash;
      if (!hex) {
        hex = h.toString('hex');
        if (!this.mutable)
          this._whhash = hex;
  ...
  getWeight() {
    const {base, witness, cost} = this.getSizes();
    const total = (base + cost) + witness;
    return (base + cost) * (consensus.WITNESS_SCALE_FACTOR - 1) + total;
  }
  ...
  signatureHash(index, prev, value, type) {
    ...
    if ((type & 0x1f) !== hashType.SINGLE
      ...
      if (index < this.outputs.length) {
        ...
        outputs = blake2b.digest(output.toRaw());
      }
    }
  ...
  checkStandard() {
    ...
    for (const output of this.outputs) {
      ...
      if (output.covenant.type > rules.MAX_COVENANT_TYPE)
  ...
  checkInputs(view, height) {
    ...
    if (!this.verifyCovenants(view, height))
  ...
  verifyCovenants(view, height) {
    return rules.verifyCovenants(this, view, height);
  }
  ...
  outpoint(index) {
    return new Outpoint(this.hash('hex'), index);
  }
  ...
  fromReader(br) {
    ...
    for (let i = 0; i < inCount; i++) {
      ...
      input.prevout.fromReader(br);
      const start = br.offset;
      input.witness.fromReader(br);
      witness += br.offset - start;
      input.sequence = br.readU32();
    ...
    if (!this.mutable) {
      ...
      const cost = this.getCovenantCost();
      this._raw = raw;
      this._sizes = new Sizes(base, witness, cost);
    } else {
      br.end();
    }
  ...
  getCovenantCost() {
    ...
    for (const {covenant} of this.outputs) {
      const {type, items} = covenant;

      // Make bids bigger so reveals can get in.
      if (type === types.BID) {
        const size = covenant.getSize();

        if (size > rules.MAX_BID_SIZE) {
          cost += size - rules.MAX_BID_SIZE;
        } else {
          cost += rules.MAX_BID_SIZE * 4;
          cost -= size;
        }
      ...
      // First updates exist as if they registered
      // 512 bytes. This limits the amount of registers
      // that can happen in one day. One year later,
      // when a renewal is required, the amount of space
      // required by renewals should be _far_ less than
      // the amount of space required by the registers.
      // Say on March 9th, 2018, there were 1500 registers.
      // That amounts to at least ~847kb of block space
      // required. On March 8th, 2019, the space required
      // to renew all of them is about ~79kb (more than
      // 10x less). The renewals for registers that
      // probably took a whole day's worth of blocks can
      // now fit into one block easily.
      if (type === types.UPDATE && items.length === 3) {
        const name = items[0];
        const data = items[1];
        const hash = items[2];
        ...
        if (name.length > rules.MAX_NAME_SIZE) {
          cost += name.length - rules.MAX_NAME_SIZE;
        } else {
          cost += encoding.sizeVarlen(rules.MAX_NAME_SIZE);
          cost -= encoding.sizeVarlen(name.length);
        }

        if (data.length > rules.MAX_RESOURCE_SIZE)
          cost += data.length - rules.MAX_RESOURCE_SIZE;
        } else {
          cost += encoding.sizeVarlen(rules.MAX_RESOURCE_SIZE);
          cost -= encoding.sizeVarlen(data.length);
        }
      }
    }
  ...
  getSizes() {
    ...
    const cost = this.getCovenantCost();
    const sizes = new Sizes(base, witness, cost);
