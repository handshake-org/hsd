...
class TX {
  ...
  signatureHash(index, prev, value, type) {
    ...
    const size = 160 + prev.getVarSize();
    const bw = bio.pool(size);
  ...
  checkStandard() {
    ...
    for (const output of this.outputs) {
      ...
      if (output.covenant.getSize() > rules.MAX_COVENANT_SIZE)
        return [false, 'covenant-size', 0];
  ...
  checkInputs(view, height) {
    ...
    if (!this.verifyCovenants(view))
      return [-1, 'bad-txns-covenants', 100];

    return [fee, 'valid', 0];
  }

  verifyCovenants(view) {
    return rules.verifyCovenants(this, view);
  }
  ...
  isWatched(filter) {
    ...
    if (filter.test(this.witnessHash()))
      found = true;

    for (let i = 0; i < this.outputs.length; i++) {
      const {address, covenant} = this.outputs[i];

      if (filter.test(address.hash)) {
    ...
    for (const {prevout, witness} of this.inputs) {
      if (filter.test(prevout.toRaw()))
        return true;

      if (witness.test(filter))
        return true;
    }
