...
class TX {
  ...
  signatureHash(index, prev, value, type) {
    ...
    const size = 157 + prev.getVarSize();
    const bw = bio.pool(size);
  ...
  getModifiedSize(size) {
    ...
    for (const input of this.inputs) {
      const offset = 41 + Math.min(110, input.witness.getSize());
      if (size > offset)
        size -= offset;
    }
  ...
  isWatched(filter) {
    ...
    for (const input of this.inputs) {
      if (filter.test(input.prevout.toRaw()))
        return true;
  ...
  fromJSON(json) {
    ...
    assert((json.type & 0xff) === json.version, 'Version must be a uint8.');
