...
class MerkleBlock extends AbstractBlock {
    ...
    const traverse = (height, pos) => {
      ...
      if (height === 0 || !parent) {
        ...
        if (height === 0 && parent) {
          ...
          indexes.push(pos);
          map.set(txid, pos);
          return rfc6962.hashLeaf(blake2b, hash);
        }
  ...
  static fromMatches(block, matches) {
    ...
    const hash = (height, pos, leaves) => {
      if (height === 0)
        return rfc6962.hashLeaf(blake2b, leaves[pos]);
    ...
    merkle.witnessRoot = block.witnessRoot;
