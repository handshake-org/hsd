...
class MTX extends TX {
  ...
  signInput(index, coin, ring, type) {
    ...
    if (addr.isScripthash()) {
      ...
      const prev = Script.fromRaw(redeem);
  ...
  isInputSigned(index, coin) {
    ...
    if (addr.isScripthash()) {
      const prev = Script.fromRaw(stack.pop());
  ...
  sortMembers() {
    ...
    for (let i = 0; i < this.outputs.length; i++) {
      ...
      if (rules.isLinked(covenant)) {
        const [input] = this.inputs.splice(i, 1);
        const [output] = this.outputs.splice(i, 1);
...
class CoinSelector {
  ...
  fromOptions(options) {
    ...
    if (options.coinbaseMaturity != null) {
      assert((options.coinbaseMaturity >>> 0) === 0);
  ...
  isSpendable(coin) {
    ...
    if (rules.isUnspendable(coin, this.height))
      return false;
