...
class Op extends bio.Struct {
  ...
  write(bw) {
    ...
    switch (this.type) {
      ...
      case types.SET_REVOKED: {
        const revoked = this.value;
        bw.writeU32(revoked >>> 0);
        break;
      }
      case types.SET_CLAIMED: {
...
class Auction extends bio.Struct {
  ...
  setRevoked(revoked) {
    ...
    this.undo.push(new Op(types.SET_REVOKED, this.revoked));
    this.ops.push(new Op(types.SET_REVOKED, revoked));
  ...
  toStats(height, network) {
    ...
    if (this.isBidding(height, network)) {
      ...
      stats.hoursUntilReveal = +hours.toFixed(2);
    }
    ...
    if (this.isReveal(height, network)) {
      ...
      stats.hoursUntilClose = +hours.toFixed(2);
    }
    ...
    if (this.isClosed(height, network)) {
      ...
      stats.daysUntilExpire = +days.toFixed(2);
    }
    ...
    if (this.isRevoked(height, network)) {
      ...
      stats.hoursUntilReopen = +hours.toFixed(2);
    }
