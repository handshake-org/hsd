...
class Auction extends bio.Struct {
  ...
  fromJSON(json) {
    ...
    assert(Number.isSafeInteger(json.value) && json.value >= -1);
    assert(Number.isSafeInteger(json.highest) && json.highest >= -1);
    assert(typeof json.data === 'string');
    assert((json.data.length & 1) === 0);
    assert(json.transfer === -1 || (json.transfer >>> 0) === json.transfer);
