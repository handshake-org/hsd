...
class Ownership extends BNSOwnership {
  ...
  createData(address, fee, commitHash, commitHeight, network) {
    ...
    const size = 2 + hash.length + bio.sizeVarint(fee) + 32 + 4 + 4;
  ...
  createSimple(address, fee, network) {
    assert(network && network.genesis);
    return this.createData(address, fee, network.genesis.hash, 0, network);
  }
}
