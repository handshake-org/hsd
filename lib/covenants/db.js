 ...
 *   h[height][name-hash] -> index (auction by height)
...
class NameDB {
  ...
  async getNamesByHeight(height) {
    return this.bucket.keys({
      gte: layout.h.min(height),
      lte: layout.h.max(height),
      parse: k => k.slice(5)
    });
  }

  async getClosedNames(height) {
    const start = height - rules.TOTAL_PERIOD;

    if (start < 0)
      return [];

    return this.getNamesByHeight(start);
  }

  async getClosedAuctions(height) {
    const hashes = await this.getClosedNames(height);
    const auctions = [];

    for (const hash of hashes) {
      const auction = await this.getAuction(hash);
      assert(auction);
    ...
    for (const op of auction.ops) {
      ...
      switch (op.type) {
        case types.ADD_AUCTION:
          ...
          b.put(layout.h.build(auction.height, nameHash), null);
          break;
        case types.REMOVE_AUCTION:
          b.del(layout.a.build(nameHash));
          b.del(layout.h.build(auction.height, nameHash), null);
          break;
        case types.ADD_BID:
