  ...
  async connectCovenants(tx, height, view) {
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
      ...
      if (uc.type === types.BID) {
        ...
        auction.addReveal(hash, i, output.value);
        auction.save();
      ...
      if (uc.type === types.REVEAL) {
        ...
        if (covenant.type === types.UPDATE) {
          auction.setOwner(hash, i);
          auction.setData(covenant.items[1]);
          auction.save();
          auction.queue();
        } else {
          auction.released = true;
          auction.save();
        }
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if (covenant.type === types.BID) {
        ...
        if (this.network.type === 'main') {
          const startHeight = (nameHash[0] % 52) * this.network.blocksPerWeek;
  ...
  async disconnectCovenants(tx, height, view) {
    ...
    for (let i = tx.inputs.length - 1; i >= 0; i--) {
      ...
      if (uc.type === types.UPDATE) {
        ...
        assert(!prevout.equals(auction.owner));
        assert(auction.owner.hash === hash && auction.owner.index === i);

        // Switch back to previous owner and data.
        auction.setOwner(prevout.hash, prevout.index);
        auction.setData(uc.items[1]);
        auction.released = false;
        auction.save();
        auction.queue();
