...
class NameDB {
  ...
  async readInputs(tx, view, height) {
    if (tx.isCoinbase())
      return true;
    ...
    for (const {prevout} of tx.inputs) {
      const {covenant} = view.getOutput(prevout);
      const {type, items} = covenant;

      if (type < types.BID || type > types.REVOKE)
        continue;

      const auction = await view.readAuctionByName(this, items[0]);

      if (!auction)
        return false;

      if (!auction.owner.isNull())
        assert(await view.readCoin(this, auction.owner));
    }
  }

  async addOutputs(tx, view, height) {
    if (tx.isCoinbase())
      return true;
    ...
    for (const {covenant} of tx.outputs) {
      const {type, items} = covenant;
      if (type === types.BID)
        await view.ensureAuction(this, items[0], height);
    }
  }

  // expiration->bids->reveals
  // revoke->bids->reveals
