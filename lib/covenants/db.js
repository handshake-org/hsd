 ...
 *   w[hash][index] -> winning outpoints
 *   u[height] -> state log
 */
...
class NameDB {
    ...
    for (const {type, params} of ops) {
      switch (type) {
        ...
        case types.SET_WINNER: {
          const [last, winner, value] = params;

          auction.winner = winner;
          auction.value = value;

          if (!last.isNull()) {
            const {hash, index} = last;
            b.del(layout.w.build(hash, index));
          }

          if (!winner.isNull()) {
            const {hash, index} = winner;
            b.put(layout.w.build(hash, index), null);
          }
  ...
  async isWinner({hash, index}) {
    return this.bucket.has(layout.w.build(hash, index));
  }
  ...
  async connect(tx, view, height) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if (auction.isNull()) {
        assert(covenant.type === types.BID, 'Database inconsistency.');
      ...
      switch (covenant.type) {
        ...
        case types.REDEEM: {
          // Edge Case:
          // If this was an output from a long
          // time ago, we want to make sure
          // someone can still get their money
          // out as long as they're not a winner.
          if (!local) {
            if (await this.isWinner(prevout))
              return false;
            break;
          }
