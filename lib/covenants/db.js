    ...
    for (const {type, params} of auction.ops) {
      switch (type) {
        ...
        case types.ADD_BID: {
          ...
          view.addOutpoint(hash, index, nameHash);
          break;
        }
        case types.REMOVE_BID: {
          ...
          view.removeOutpoint(hash, index);
          break;
        }
        ...
        case types.ADD_OUTPOINT: {
          const [{hash, index}] = params;
          b.put(layout.n.build(hash, index), nameHash);
          view.addOutpoint(hash, index, nameHash);
          break;
        }
        case types.REMOVE_OUTPOINT: {
          const [{hash, index}] = params;
          b.del(layout.n.build(hash, index));
          view.removeOutpoint(hash, index);
          break;
  ...
  async connectCovenants(tx, height, view) {
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
      ...
      if (uc.type === types.TRANSFER) {
        ...
        // Must be the owner to update.
        // Might not need .transfer, because we don't technically need these checks!
        if (!prevout.equals(auction.transfer))
          return false;
        ...
        switch (covenant.type) {
          ...
          case types.UPDATE: {
            auction.setOwner(outpoint);
            auction.setTransfer(new Outpoint());
            auction.commit(covenant.items[1]);
            auction.save();
            break;
          }
      ...
      if (covenant.type === types.BID) {
        ...
        if (auction.bids === 0) {
          ...
          if (undo) {
            ...
            auction.setTransfer(undo.transfer);
    ...
    for (let i = tx.inputs.length - 1; i >= 0; i--) {
      ...
      if (uc.type === types.REVEAL) {
        ...
        switch (covenant.type) {
          ...
          case types.TRANSFER: {
            assert(auction.state(height, network) === states.CLOSED);
            assert(auction.transfer.equals(outpoint));
            auction.addReveal(prevout, coin.value, entry.height);
            auction.setTransfer(new Outpoint());
            auction.renewal = auction.height;
            auction.save();
