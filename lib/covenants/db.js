...
class NameDB {
  ...
  async check(tx, view, height) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      // Edge case:
      // The output used to create the first
      // cold output is the only covenant
      // creation that is allowed to be
      // non-local to the auction.
      let firstCold = false;
      ...
      if (covenant.type === types.COLD) {
        ...
        if (coin) {
          ...
          if (uc.type !== types.COLD)
            firstCold = true;
        } else {
          // Revalidation in the
          // mempool doesn't matter.
          firstCold = true;
        }
      }
      ...
      if (!local && !firstCold)
        return [false, 'bad-auction-nonlocal'];
