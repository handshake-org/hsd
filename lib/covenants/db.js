...
class NameDB {
  ...
  async getAuction(nameHash) {
    ...
    return Auction.fromRaw(raw);
  }

  async getAuctionHash(prevout) {
    const {hash, index} = prevout;
    const nameHash = await this.bucket.get(layout.n(hash, index));

    if (!nameHash)
      return null;

    return nameHash.toString('hex');
  }
  ...
  saveAuction(b, auction) {
    const nameHash = Buffer.from(auction.nameHash, 'hex');
    ...
    for (const op of auction.ops) {
      ...
      switch (op.code) {
        ...
        case types.ADD_REVEAL:
          b.put(layout.r.build(nameHash, hash, index), fromU64(op.value));
          b.put(layout.n.build(hash, index), nameHash);
          break;
  ...
  async connectNames(tx, height, view) {
    ...
    for (const input of tx.inputs) {
      ...
      if (uc.type === types.BID) {
        const nameHash = blake2b.digest(uc.items[0]).toString('hex');
      ...
      if (uc.type === types.REVEAL) {
        ...
        auction.setData(covenant.items[0]);
        auction.save();
        auction.queue();
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if (covenant.type === types.BID) {
        ...
        const nameHash = blake2b.digest(name).toString('hex');
        const auction =
