...
function readTarget(type, br, d) {
  switch (type) {
    ...
    case rtypes.INAME:
      return verifyName(d.read(br), false);
    case rtypes.HNAME:
      return verifyName(d.read(br), true);
    default:
      throw new Error('Unknown target type.');
  }
}
...
function verifyName(name, hsk) {
  for (let i = 0; i < name.length; i++) {
    ...
    if (ch === '.') {
      ...
      if (i === name.length - 1)
        throw new Error('Unexpected FQDN.');
...
class Target extends RecordData {
  ...
  fromString(str) {
    ...
    if (onion.isString(str)) {
      this.type = rtypes.ONION;
      this.target = onion.normalize(str);
      return this;
    }

    if (onion.isNGString(str)) {
      this.type = rtypes.ONIONNG;
      this.target = onion.normalizeNG(str);
      return this;
    }
    ...
    assert(verifyName(str));

    if (util.endsWith(str, HSKP)) {
      this.type = rtypes.HNAME;
      this.target = str.slice(0, -HSKP.length);
    } else if (util.endsWith(str, ICANNP)) {
      this.type = rtypes.INAME;
      this.target = str.slice(0, -ICANNP.length);
    } else {
      this.type = rtypes.INAME;
      this.target = str;
    }
  ...
  toPointer(name) {
    ...
    let ip = IP.toBuffer(this.target);

    if (IP.isIPv4(ip))
      ip = ip.slice(12, 16);

    const hash = base32.encode(ip);
