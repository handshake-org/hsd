...
class HSKRecord extends RecordData {
  ...
  toSOA(name) {
    ...
    rr.name = util.fqdn(top);
    rr.type = types.SOA;
    rr.ttl = 900;
    rr.data = rd;
    rd.ns = util.fqdn(top); // Need to put an ns here
    rd.mbox = HSKS;
    rd.serial = 0;
    rd.refresh = 1800;
    rd.retry = 900;
  ...
  toDNS(name, type, naked) {
    ...
    if (labels.length > 1) {
      const next = name.substring(labels[labels.length - 1]);

      if (this.ns.length > 0) {
        res.authority = this.toNS(next, naked);
        res.additional = this.toNSIP(next, naked);
      } else if (this.delegate) {
        // Should be in answer???
        res.answer = this.toDNAME(next);
      } else {
        res.authority = this.toSOA(next);
      }

      // Always push on DS records for a referral.
      for (const rr of this.toDS(next))
    ...
    // Use CNAME and A records (_direct_ non-referral records).
    if (this.canonical) {
    ...
    // NOTE TO SELF: Write faux recrusive server using remote stub servers,
    // request NS when we need to, explicitly, etc. Could verify DNSSEC?
    // Tired, going to sleep now.
    // Last comment before bed:
    // Parse domain incrementally to hit authoritative servers first --
    // avoid recursion? Possibly set RD bit to zero when needed?
    switch (type) {
      case types.ANY:
...
r.url.push('https://google.com/');
r.url.push('https://google.com/');
r.email.push('example1@google.com');
r.text.push('google it');
r.magnet.push(new Magnet('btih', '0000000000000000000000000000000000000000'));
r.ds.push(new DS());
r.tls.push(new TLS());
