...
class FullNode extends Node {
  ...
  async sendClaim(claim) {
    try {
      ...
      if (err.type === 'VerifyError' && err.score === 0) {
        this.error(err);
        this.logger.warning('Verification failed for claim: %s.', claim.hash());
        this.logger.warning('Attempting to broadcast anyway...');
  ...
  async getAuctionState(nameHash) {
    ...
    const cdb = this.chain.cdb;
    const height = this.chain.height + 1;
    const hardened = await this.mempool.hasHardening();
    const state = await cdb.getAuctionState(nameHash, height, hardened);

    return state.getJSON(height, network);
  }
}
