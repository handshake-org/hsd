...
class FullNode extends Node {
  ...
  async sendClaim(claim) {
    try {
      ...
      if (err.type === 'VerifyError' && err.score === 0) {
        this.error(err);
        this.logger.warning(
          'Verification failed for claim: %s.',
          claim.hash('hex'));
        this.logger.warning('Attempting to broadcast anyway...');
  ...
  async getAuctionState(nameHash, height, hardened) {
    return this.chain.cdb.getAuctionState(nameHash, height, hardened);
  }
  ...
  async isBiddable(nameHash) {
    ...
    const {state} = await this.getAuctionState(nameHash, height, hardened);
