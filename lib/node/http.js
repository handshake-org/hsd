...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/block/:block', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const hash = valid.uinthash('block');
  ...
  handleAuth(socket) {
    ...
    socket.hook('send claim', (...args) => {
      ...
      const claim = Claim.fromRaw(data);
    ...
    socket.hook('get auction', async (...args) => {
      const valid = new Validator(args);
      const nameHash = valid.buf(0);
      const height = valid.u32(1, 0);
      const hardened = valid.bool(2, false);

      if (!nameHash || nameHash.length !== 32)
        throw new Error('Invalid parameter.');

      return this.node.getAuctionState(nameHash, height, hardened);
    });

    socket.hook('is biddable', async (...args) => {
      const valid = new Validator(args);
      const nameHash = valid.buf(0);
      ...
      return this.node.isBiddable(nameHash);
    });

    socket.hook('rescan', (...args) => {
      const valid = new Validator(args);
      const start = valid.uinthash(0);
