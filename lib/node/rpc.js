...
class RPC extends RPCBase {
  ...
  async getNetworkInfo(args, help) {
    ...
    return {
      ...
      protocolversion: this.pool.options.version,
      identitykey: this.pool.getKey('base32'),
      identitykeyhex: this.pool.getKey('hex'),
  ...
  async getPeerInfo(args, help) {
    ...
    for (let peer = this.pool.peers.head(); peer; peer = peer.next) {
      ...
      peers.push({
        ...
        type: peer.peerType(),
        identitykey: peer.address.hasKey()
          ? peer.address.getKey('base32')
          : null,
        identitykeyhex: peer.address.hasKey()
          ? peer.address.getKey('hex')
          : null,
