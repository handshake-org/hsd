...
class RPC extends RPCBase {
  ...
  async _handleWork(data) {
    ...
    const prevBlock = data.slice(32, 64);
    const maskHash = data.slice(96, 128);
    const bits = bio.readU32(data, 252);

    if (!prevBlock.equals(attempt.prevBlock)
        || bits !== attempt.bits) {
      return [false, 'stale'];
    }

    const mask = this.maskMap.get(maskHash);

    if (!mask)
      return [false, 'stale'];

    const nonce = bio.readU32(data, 0);
    const time = bio.readU64(data, 4);
    const extraNonce = data.slice(128, 160);
