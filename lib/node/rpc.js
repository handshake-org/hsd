...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('isnameavailable', this.isNameAvailable);
  ...
  async getPeerInfo(args, help) {
    ...
    for (let peer = this.pool.peers.head(); peer; peer = peer.next) {
      ...
      peers.push({
        ...
        besthash: peer.bestHash ? peer.bestHash : null,
  ...
  async getBlockchainInfo(args, help) {
    ...
    return {
      ...
      bestblockhash: this.chain.tip.hash,
      difficulty: toDifficulty(this.chain.tip.bits),
  ...
  async getTXOutProof(args, help) {
    ...
    for (const hash of hashes) {
      ...
      whashes.push(tx.witnessHash());
    }

    const mblock = MerkleBlock.fromHashes(block, whashes);

    return mblock.toRaw().toString('hex');
  }
  ...
  async _handleWork(data) {
    ...
    const header = Headers.fromRaw(data);
  ...
  async _createWork() {
    ...
    const data = attempt.getPreheader(root, time, consensus.ZERO_NONCE);
  ...
  async _createTemplate(maxVersion, coinbase, rules) {
    ...
    const json = {
      ...
      vbrequired: 0,
      height: attempt.height,
      previousblockhash: attempt.prevBlock,
      merkleroot: attempt.merkleRoot,
      treeroot: attempt.treeRoot,
      target: attempt.target.toString('hex'),
      cuckoo: this.network.cuckoo,
      ...
      longpollid: this.chain.tip.hash + hex32(this.totalTX()),
      submitold: false,
      coinbaseaux: {
  ...
  async createRawTransaction(args, help) {
    ...
    for (const key of Object.keys(sendTo)) {
      if (key === 'data') {
        ...
        output.value = 0;
        output.address.version = 255;
        output.address.hash = value;
  ...
  async verifyMessage(args, help) {
    ...
    return ccmp(blake2b.digest(key, 20), addr.hash);
  }
  ...
  async isNameAvailable(args, help) {
    if (help || args.length !== 1)
      throw new RPCError(errs.MISC_ERROR, 'isnameavailable "name"');
    ...
    return this.chain.cdb.isAvailable(name, height);
  }
  ...
  async getNameInfo(args, help) {
    ...
    const [start, week] = rules.getRollout(name, this.network);
    const auction = await this.chain.cdb.getAuctionByName(name);
    const height = this.chain.height + 1;
    ...
    if (auction) {
      info = auction.getJSON(this.chain.height, this.network);
      if (auction.data) {
        try {
          const res = Resource.fromRaw(auction.data);
          resource = res.toJSON();
        } catch (e) {
          ;
        }
    ...
    return {
      start: {
        reserved: rules.isReserved(name, height, this.network),
        week: week,
        start: start
      },
      auction: info,
  ...
  async getNameProof(args, help) {
    ...
    return {
      ...
      nodes: proof.nodes.map(n => n.toString('hex')),
      value: proof.value ? proof.value.toString('hex') : undefined,
      nx_key: proof.key ? proof.key.toString('hex') : undefined,
      nx_hash: proof.hash ? proof.hash.toString('hex') : undefined
    };
  }
  ...
  async getNameByHash(args, help) {
    ...
    const hash = valid.buf(0, null, 'hex');

    if (!hash || hash.length !== 20)
      throw new RPCError(errs.TYPE_ERROR, 'Invalid name hash.');

    const auction = await this.chain.cdb.getAuction(hash);

    if (!auction)
      return null;

    return auction.name.toString('ascii');
  }
  ...
  async _addBlock(block) {
    this.logger.info('Handling submitted block: %s.', block.hash('hex'));
  ...
  async headerToJSON(entry) {
    ...
    return {
      ...
      witnessroot: entry.witnessRoot,
      treeroot: entry.treeRoot,
      solution: entry.solution.toJSON(),
      time: entry.time,
      mediantime: mtp,
      bits: entry.bits,
