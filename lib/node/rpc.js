...
const {TrieProof} = require('../trie/proof');
const {EXP} = consensus;
...
class RPC extends RPCBase {
  ...
  async decodeScript(args, help) {
    ...
    const addr = Address.fromScripthash(script.hash256());
  ...
  async getNameStatus(args, help) {
    ...
    const [start, week] = rules.getRollout(name);
    const auction = await this.chain.cdb.getAuctionByName(name);
    const data = await this.chain.cdb.getDataByName(name);
    const height = this.chain.height + 1;
  ...
  async getNameProof(args, help) {
    ...
    const [key, nodes] = await this.chain.cdb.proveName(root, name);
    const data = await this.chain.cdb.getDataByName(name);
