...
class RPC extends RPCBase {
  ...
  async estimateFee(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR,
        'estimatefee "nblocks" ( "covenantType" )');

    const valid = new Validator(args);
    const blocks = valid.u32(0, 1);
    const val = valid.str(1, 'NONE');

    if (!this.fees)
      throw new RPCError(errs.MISC_ERROR, 'Fee estimation not available.');

    const type = rules.types[val];
    if (typeof type !== 'number')
      throw new RPCError(errs.MISC_ERROR, `${val} not a valid covenant type.`);

    const fee = this.fees.estimateFee(blocks, false, type);
  ...
  async estimatePriority(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR,
        'estimatepriority "nblocks" ( "covenantType" )');
    ...
    return this.fees.estimatePriority(blocks, false, type);
  }

  async estimateSmartFee(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR,
        'estimatesmartfee "nblocks" ( "covenantType" )');
    ...
    let fee = this.fees.estimateFee(blocks, true, type);
  ...
  async estimateSmartPriority(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR,
        'estimatesmartpriority "nblocks" ( "covenantType" )');
    ...
    const pri = this.fees.estimatePriority(blocks, true, type);
