...
class RPC extends RPCBase {
  ...
  async decodeScript(args, help) {
    ...
    const addr = Address.fromScripthash(script.blake256());
  ...
  async getNameProof(args, help) {
    ...
    const nodes = await this.chain.db.prove(root, key);
    ...
    return {
      ...
      nodes: nodes.map(n => n.toString('hex'))
    };
  }
