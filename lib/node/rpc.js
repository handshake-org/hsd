...
class RPC extends RPCBase {
  ...
  async _createTemplate(maxVersion, coinbase, rules) {
    ...
    const json = {
      ...
      merkleroot: util.revHex(attempt.merkleRoot),
      trieroot: util.revHex(attempt.trieRoot),
  ...
  async headerToJSON(entry) {
    ...
    return {
      ...
      witnessroot: util.revHex(entry.witnessRoot),
      trieroot: util.revHex(entry.trieRoot),
