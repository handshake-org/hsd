...
class RPC extends RPCBase {
  ...
  async _createTemplate(maxVersion, coinbase, rules) {
    ...
    const json = {
      ...
      reservedroot: util.revHex(attempt.reservedRoot),
      target: util.revHex(attempt.target.toString('hex')),
      cuckoo: {
        bits: this.network.cuckoo.bits,
        size: this.network.cuckoo.size,
        ease: this.network.cuckoo.ease
      },
  ...
  async headerToJSON(entry) {
    ...
    return {
      ...
      reservedroot: util.revHex(entry.reservedRoot),
  ...
  async blockToJSON(entry, block, details) {
    ...
    return {
      ...
      reservedroot: util.revHex(entry.reservedRoot),
      solution: entry.solution.toJSON(),
      coinbase: block.txs[0].inputs[0].witness.toJSON(),
      tx: txs,
      time: entry.time,
      mediantime: mtp,
      bits: entry.bits,
