...
class RPC extends RPCBase {
  ...
  async _createTemplate(maxVersion, coinbase, rules) {
    ...
    const json = {
      ...
      reservedroot: attempt.reservedRoot,
      target: attempt.target.toString('hex'),
  ...
  async getNameInfo(args, help) {
    ...
    const nextHeight = this.chain.height + 1;

    let info = null;

    if (auction) {
      if (!auction.isExpired(height, network))
        info = auction.getJSON(height, network);
    }

    return {
      start: {
        reserved: rules.isReserved(name, nextHeight, network),
        week: week,
        start: start
      },
      auction: info
  ...
  async headerToJSON(entry) {
    ...
    return {
      ...
      reservedroot: entry.reservedRoot,
