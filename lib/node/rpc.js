...
class RPC extends RPCBase {
  ...
  async getWork(args, help) {
    if (help || args.length > 1)
      throw new RPCError(errs.MISC_ERROR, 'getwork ( "merkleroot" )');
  ...
  async getNameInfo(args, help) {
    ...
    const reserved = rules.isReserved(name, height + 1, network);
    const [start, week] = rules.getRollout(name, network);
