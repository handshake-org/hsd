...
class BlockTemplate {
  ...
  refresh() {
    ...
    this.merkleRoot = merkle.createRoot(BLAKE2b256, leaves);
  ...
  getProof(nonce1, nonce2, time, nonce) {
    ...
    const hdr = this.getHeader(root, time, nonce);
  ...
  commit(proof) {
    ...
    block.witnessRoot = root;
    block.treeRoot = this.treeRoot;
    block.filterRoot = this.filterRoot;
  ...
  toBlock() {
    ...
    const proof = this.getProof(0, 0, this.time, nonce);
  ...
  addAirdrop(proof) {
    const entry = BlockClaim.fromAirdrop(proof);
...
class BlockProof {
  ...
  hash() {
    return BLAKE2b256.digest(this.hdr);
  }
...
class MerkleTree {
  ...
  withFirst(hash) {
    let root = merkle.hashLeaf(BLAKE2b256, hash);

    for (const step of this.steps)
      root = merkle.hashInternal(BLAKE2b256, root, step);
  ...
  fromLeaves(leaves) {
    ...
    const sentinel = merkle.hashEmpty(BLAKE2b256);

    for (const hash of leaves) {
      const leaf = merkle.hashLeaf(BLAKE2b256, hash);
      nodes.push(leaf);
    }
    ...
    while (len > 1) {
      ...
      for (let i = 2; i < len; i += 2) {
        ...
        const hash = merkle.hashInternal(BLAKE2b256, left, right);
        hashes.push(hash);
      }
