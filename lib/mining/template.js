...
class BlockTemplate {
  ...
  getHeader(nonce, time, extraNonce, mask) {
    const maskHash = BLAKE2b.multi(this.prevBlock, mask);
    const bw = bio.write(128 + 128);
    const pad20 = Buffer.alloc(20);

    for (let i = 0; i < 20; i++)
      pad20[i] = this.prevBlock[i] ^ this.treeRoot[i];
    ...
    bw.writeHash(maskHash); // <-- replace commitment hash with mask hash.

    // Subheader.
    bw.writeBytes(extraNonce);
...
class BlockProof {
  ...
  shareHash() {
    const prevBlock = this.hdr.slice(32, 64);
    const treeRoot = this.hdr.slice(64, 96);
    const preHead = Buffer.from(this.hdr.slice(0, 128));
    const subHead = Buffer.from(this.hdr.slice(128, 256));

    this.extraNonce.copy(subHead, 0);

    const subHash = BLAKE2b.digest(subHead);
    const maskHash = BLAKE2b.multi(prevBlock, this.mask);
    const commitHash = BLAKE2b.multi(subHash, maskHash);

    bio.writeU32(preHead, this.nonce, 0);
    bio.writeU64(preHead, this.time, 4);
    ...
    return BLAKE2b.multi(left, pad32, right);
  }
