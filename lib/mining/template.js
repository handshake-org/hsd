...
class BlockTemplate {
  ...
  createCoinbase() {
    ...
    for (const claim of this.claims) {
      ...
      output.covenant.items.push(Buffer.from(claim.nameHash, 'hex'));
      output.covenant.items.push(Buffer.from(claim.name, 'ascii'));
...
class BlockClaim {
  ...
  static fromClaim(claim, data) {
    ...
    item.nameHash = rules.hashName(data.name).toString('hex');
    item.name = data.name;
  ...
  fromLeaves(leaves) {
    ...
    while (len > 1) {
      ...
      for (let i = 2; i < len; i += 2) {
        ...
        if (i + 1 < len)
          hash = rfc6962.hashInternal(blake2b, nodes[i], nodes[i + 1]);
        else
          hash = nodes[i];
