...
const rfc6962 = require('../protocol/rfc6962-2');
...
class BlockTemplate {
  ...
  createCoinbase() {
    ...
    for (const claim of this.claims) {
      ...
      output.covenant.items.push(claim.nameHash);
      output.covenant.items.push(encodeU32(this.height));
      output.covenant.items.push(claim.name);
...
class MerkleTree {
  ...
  withFirst(hash) {
    let root = rfc6962.hashLeaf(blake2b, hash);

    for (const step of this.steps)
      root = rfc6962.hashInternal(blake2b, root, step);
  ...
  fromLeaves(leaves) {
    ...
    const sentinel = rfc6962.hashEmpty(blake2b);

    for (const hash of leaves) {
      const leaf = rfc6962.hashLeaf(blake2b, hash);
      nodes.push(leaf);
    }
    ...
    while (len > 1) {
      ...
      for (let i = 2; i < len; i += 2) {
        ...
        const hash = rfc6962.hashInternal(blake2b, left, right);
        hashes.push(hash);
      }
