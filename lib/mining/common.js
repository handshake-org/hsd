...
common.double256 = function double256(target) {
  ...
  hi = target.readUInt32LE(0, true);
  lo = target.readUInt32LE(4, true);
  n += (hi * 0x100000000 + lo) * B192;

  hi = target.readUInt32LE(8, true);
  lo = target.readUInt32LE(12, true);
  n += (hi * 0x100000000 + lo) * B128;

  hi = target.readUInt32LE(16, true);
  lo = target.readUInt32LE(20, true);
  n += (hi * 0x100000000 + lo) * B64;

  hi = target.readUInt32LE(24, true);
  lo = target.readUInt32LE(28, true);
