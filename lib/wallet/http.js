...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.post('/wallet/:id/batch/reveal', async (req, res) => {
      ...
      const processedReveals = [];
      let errorMessages = [];

      for (const name of names) {
        const cachedResponse = revealCache.get(name);
        if (cachedResponse) {
          cachedResponse.forEach(cachedReveal => processedReveals.push(cachedReveal));
        }
      }

      // Not discriminating between names, to let
      // wallet to process half revealed names if any
      try {
        const batchRevealResponse = await req.wallet
          .createBatchReveal(names, options);

        const mtx = batchRevealResponse.mtx;
        errorMessages = batchRevealResponse.errorMessages;
        ...
        for (let i=0; i<outputs.length; i++) {
          ...
          const cachedItem = Object.assign({}, processedReveal);
          cachedItem.from_cache = true;

          if (revealCache.has(name)) {
            revealCache.get(name).push(cachedItem);
          } else {
            revealCache.set(name, [cachedItem])
          }
      }

      } catch(err) {
        // TODO looks bad i know, but it's for letting a half revealed domain
        // to complete it's reveals
        if (names.length === 1) {
          if (!err.message || err.message !== `No bids to reveal: "${names[0]}".`) 
              throw err;          
        } else {
          if (!err.message || err.message !== 'Invalid (Batch) Reveal Request') 
            throw err;
        }       
      }

      return res.json(200, {processedReveals, errorMessages});
    });
    /* eslint-enable */
