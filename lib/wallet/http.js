...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.post('/wallet/:id/batch/open', async (req, res) => {
      ...
      assert(names.length <= MAX_NAME_ARRAY_LENGTH, `Names array can nt contain more than ${MAX_NAME_ARRAY_LENGTH}`);
      assert(broadcast ? sign : true, 'Must sign when broadcasting.');

      const options = TransactionOptions.fromValidator(valid);
      const {mtx, errors, isAllError} = await req.wallet.createBatchOpen(names, force, options);
