...
class Wallet extends EventEmitter {
  ...
  async makeRevealAll() {
    ...
    // Just locktime and version to start
    const sizes = mtx.getSizes();
    // Add buffer for funding and change input/output
    sizes.base += 150;
    ...
    for (const {nameHash, prevout, own} of bids) {
      ...
      // Make sure this TX isn't getting too big
      sizes.base += output.getSize();
      // Every input adds hash: 32, index: 4, sequence: 4
      sizes.base += 40;
      // Witness size depends on spend type
      sizes.witness += await this.estimateSize(coin.address);
      if (sizes.getWeight() > policy.MAX_TX_WEIGHT) {
        this.logger.warning('Can not reveal all due to policy weight limit.');
        break;
      }
  ...
  async makeRedeemAll() {
    ...
    for (const {nameHash, prevout, own} of reveals) {
      ...
      sizes.witness += await this.estimateSize(coin.address);
      if (sizes.getWeight() > policy.MAX_TX_WEIGHT) {
        this.logger.warning('Can not redeem all due to policy weight limit.');
        break;
      }
