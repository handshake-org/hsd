...
const {encodeU32} = require('../utils/util');
const {types} = rules;
const {Mnemonic} = HD;
...
class Wallet extends EventEmitter {
  ...
  async makeOpen(name, acct) {
    ...
    let auction = await this.getAuction(nameHash);

    if (!auction)
      auction = await this.wdb.getAuctionStatus(nameHash);
  ...
  async makeFinalize(name) {
    ...
    const addr = coin.covenant.items[3];
