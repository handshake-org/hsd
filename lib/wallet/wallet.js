...
const {HSKResource} = require('../covenants/record');
const {types} = rules;
const {Mnemonic} = HD;
...
class Wallet extends EventEmitter {
  ...
  async makeBid(name, value, lockup, acct) {
    ...
    if (!rules.verifyRollout(name, this.wdb.height + 1, this.network))
      throw new Error('Name not yet available.');

    const auction = await this.getAuction(name);

    if (auction && auction.state !== types.BID)
  ...
  async makeRedeem(name) {
    ...
    if (auction.state === types.BID)
      throw new Error('Cannot register (bad state).');

    const reveals = await this.txdb.getReveals(name);
    ...
    for (const {prevout, own} of reveals) {
      ...
      if (!coin)
        throw new Error('Coin not found for redeem.');
    ...
    if (mtx.outputs.length === 0)
      throw new Error('Cannot redeem (no outputs).');
  ...
  async _makeRegister(name, resource) {
    ...
    if (!resource)
      resource = new HSKResource();

    assert(resource instanceof HSKResource);
    ...
    if (auction.state !== types.REVEAL)
      throw new Error('Cannot update (bad state).');

    assert(!owner.isNull());

    const [value, owner] = await this.txdb.getWinningReveal(name);
    ...
    output.covenant.items.push(consensus.ZERO_HASH);
  ...
  async _makeUpdate(name, resource) {
    assert(typeof name === 'string');
    assert(!resource || (resource instanceof HSKResource));
   ...
   * Make a transfer MTX.
   * @param {String} name
   * @param {Address} hotAddress
   * @param {Address} coldAddress
   * @returns {MTX}
   */

  async makeTransfer(name, hotAddress, coldAddress) {
    assert(typeof name === 'string');
    assert(hotAddress instanceof Address);
    assert(coldAddress instanceof Address);
    ...
    next.address = coldAddress;
    next.value = 0;
    next.covenant.type = types.COLD;
    next.covenant.items.push(raw);

    const mtx = new MTX();
    mtx.addOutpoint(auction.owner);
    mtx.addOutpoint(auction.cold);
    mtx.outputs.push(output);
    mtx.outputs.push(next);
  ...
  async createTransfer(name, hotAddress, coldAddress, options) {
    const mtx = await this.makeTransfer(name, hotAddress, coldAddress);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }
  ...
  async _sendTransfer(name, hotAddress, coldAddress, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this.createTransfer(name, hotAddress, coldAddress, options);
    return this.sendMTX(mtx, passphrase);
  }
  ...
  async sendTransfer(name, hotAddress, coldAddress, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendTransfer(name, hotAddress, coldAddress, options);
    } finally {
      unlock();
    }
  }
