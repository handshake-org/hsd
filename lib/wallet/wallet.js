...
class Wallet extends EventEmitter {
  ...
  async sendBid(name, bid, value, options) {
    if (!options)
      options = {};
    const mtx = await this.createBid(name, bid, value, options);
  ...
  async createRegister(name, data, options, renew = false) {
    ...
    if (renew) {
      ...
      // Commit to a block within the past 6 months,
      // to prove that we still own the key. This
      // prevents people from presigning 1000 years
      // worth of renewals. The block must be at
      // least 400 blocks back to prevent the
      // possibility of a reorg invalidating the
      // covenant.
      const height = this.wdb.height - consensus.COINBASE_MATURITY;

      // Chain does not allow renewals yet.
      if (height < 0)
        throw new Error('Cannot renew yet.');
  ...
  async sendRegister(name, data, options) {
    if (!options)
      options = {};
    const mtx = await this.createRegister(name, data, options);
    return this.__send(mtx, options.passphrase);
  }
  ...
  async sendRenewal(name, options) {
    if (!options)
      options = {};
    const mtx = await this.createRenewal(name, data, options);
    return this.__send(mtx, options.passphrase);
  }
