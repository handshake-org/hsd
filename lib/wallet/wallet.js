/*!
 * wallet.js - wallet object for hsk
...
const {states} = require('../covenants/auction');
const {types} = rules;
const {Mnemonic} = HD;
...
class Wallet extends EventEmitter {
   ...
   * based on account address, name hash,
   * and bid value.
  ...
  async generateNonceFromAddress(address, nameHash, value) {
    const hash = Address.getHash(address, 'hex');
    const path = await this.getPath(hash);
    ...
    return this.generateNonce(path.account, nameHash, value);
  }

  /**
   * Generate nonce deterministically
   * based on account pubkey, name hash,
   * and bid value.
   * @param {Number} acct
   * @param {Buffer} nameHash
   * @param {Amount} value
  ...
  async generateNonce(acct = 0, nameHash, value) {
    const account = await this.getAccount(acct);
    ...
    return blake2b.multi(publicKey, nameHash);
  }
  ...
  async makeClaim(name) {
    ...
    const addr = Address.fromHash(network.keys.claimant, 0);

    if (!await this.hasAddress(addr))
      throw new Error('Wallet is not a claimant.');
  ...
  /**
   * Create and finalize a claim MTX.
   * @param {String} name
   * @param {Object} options
   * @returns {MTX}
   */
  ...
  async createClaim(name, options) {
    const mtx = await this.makeClaim(name);
  ...
  async _sendClaim(name, options) {
    ...
    const mtx = await this.createClaim(name);
  ...
  async makeBid(name, value, lockup, acct) {
    ...
    const nonce = await this.generateNonce(acct, nameHash, value);
  ...
  async makeReveal(name) {
    ...
    if (state !== states.BIDDING && state !== states.REVEAL)
  ...
  async _makeRegister(name, resource) {
    ...
    const {hash, index} = auction.owner;
    const coin = await this.getCoin(hash, index);
    ...
    if (coin.covenant.type !== types.REVEAL)
      throw new Error('Name must be in REVEAL state.');
    ...
    output.covenant.items.push(nameHash);
    output.covenant.items.push(resource.toRaw());
    output.covenant.items.push(await this.wdb.getRenewalBlock());
  ...
  async _makeUpdate(name, resource) {
    assert(typeof name === 'string');
    ...
    if (resource)
      ...
      output.covenant.items.push(Buffer.alloc(0));
  ...
  async makeUpdate(name, resource) {
    assert(typeof name === 'string');
    assert(resource instanceof Resource);
    return this._makeUpdate(name, resource);
  }
  ...
  async makeRenewal(name) {
    assert(typeof name === 'string');
    return this._makeUpdate(name, null);
  }
  ...
  async makeTransfer(name, address) {
    ...
    if (!auction.isMature(height, network))
      throw new Error('Auction is not yet mature.');
  ...
  async makeCancel(name, resource) {
    assert(typeof name === 'string');
    assert(!resource || (resource instanceof Resource));
  ...
  async createCancel(name, resource, options) {
    const mtx = await this.makeCancel(name, resource);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }
  ...
  async _sendCancel(name, resource, options) {
    const passphrase = options ? options.passphrase : null;
  ...
  async sendCancel(name, resource, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendCancel(name, resource, options);
    } finally {
      unlock();
    }
  }
  ...
  async makeFinalize(name, resource) {
    assert(typeof name === 'string');
    assert(!resource || (resource instanceof Resource));
  ...
  async createFinalize(name, resource, options) {
    const mtx = await this.makeFinalize(name, resource);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }
  ...
  async _sendFinalize(name, resource, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this.createFinalize(name, resource, options);
    return this.sendMTX(mtx, passphrase);
  }
  ...
  async sendFinalize(name, resource, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendFinalize(name, resource, options);
    } finally {
      unlock();
    }
  }
  ...
  async getAuction(nameHash) {
    return this.txdb.getAuction(nameHash);
  }
  ...
  async getAuctionByName(name) {
    return this.txdb.getAuction(rules.hashName(name));
  }
  ...
  async getBidsByName(name) {
    return this.txdb.getBids(rules.hashName(name));
  }
  ...
  async getRevealsByName(name) {
    return this.txdb.getReveals(rules.hashName(name));
  }
