...
class Wallet extends EventEmitter {
  ...
  async _createClaim(name, options) {
    ...
    try {
      ...
      this.logger.debug("Failed to generate proof");
    }
  ...
  async makeRevealForAccount(name, accountName) {
    assert(typeof name === "string");
    ...
    for (const { prevout, own } of bids) {
      ...
      const { hash, index } = prevout;
      const coin = await this.getCoin(hash, index);
      const account = await this.getAccountByAddress(coin.address);
  ...
  /**
   * Create and finalize a bid MTX for an account.
   * @param {String} name
   * @param {Object} options
   * @returns {MTX}
   */

  async createRevealForAccount(name, account, options) {
    const mtx = await this.makeRevealForAccount(name, account);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }
  ...
  async _sendRevealForAccount(name, account, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this.createRevealForAccount(name, account, options);
    return this.sendMTX(mtx, passphrase);
  }
  ...
  async sendRevealForAccount(name, account, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendRevealForAccount(name, account, options);
    } finally {
      unlock();
    }
  }
