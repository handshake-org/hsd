...
class Wallet extends EventEmitter {
  ...
  async makeBatchReveal(names, acct) {
    ...
    // TODO refactor
    // we can create an output with a limited size only
    const {validDomains, rejectedDomains} = util.createBatch(200, outputMap);
    const mtx = new MTX();
    for (const validDomain of validDomains) {
      const outputs = outputMap.get(validDomain);
      for (const {outpoint, output} of outputs) {
        mtx.addOutpoint(outpoint);
        mtx.outputs.push(output);
      }
    }
    for (const rejectedDomain of rejectedDomains) {
      errorMessages.push({name: rejectedDomain,
        error: `Not processing ${rejectedDomain} since cumulative output size is exceeding allowed value`});
    }
