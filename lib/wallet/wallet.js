...
class Wallet extends EventEmitter {
  ...
  async processDomainNameForReveal(domainName, acct) {
  ...
  async makeBatchReveal(names, acct) {
    ...
    for (const domainName of names) {
        try {
          const outputs = await this.processDomainNameForReveal(domainName, acct);
          if (outputs.length === 0) {
    ...
    for (const [_, outputs] of outputMap) {
  ...
  async _createBatchReveal(names, options) {
    ...
    await this.fill(mtx, options);
    const finalizedMtx = this.finalize(mtx, options);
    return {mtx: finalizedMtx, errorMessages};
  }
