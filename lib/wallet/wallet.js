...
class Wallet extends EventEmitter {
  ...
  async _createClaim(name, options) {
    ...
    if (auction) {
      ...
      if (!await this.wdb.isBiddable(nameHash))
    ...
    if (proof) {
      ...
      // RRSIG record size.
      if (z.length < 2 || z[z.length - 1].claim.length === 0) {
        added += item.target.length; // rrname
        added += 10; // header
    ...
    const minFee = options.fee || policy.getMinFee(size, rate);
    const fee = Math.min(item.value, minFee);

    const renewal = await this.wdb.getRenewalBlock();
    const block = renewal.toString('hex');

    const acct = options.account || 0;
    const address = await this.receiveAddress(acct);
    const txt = ownership.createData(fee, block, address, false, network);
  ...
  async _sendClaim(name, options) {
    const claim = await this.makeClaim(name);
  ...
  async makeTransfer(name, address) {
    ...
    if (auction.weak && auction.height < height + 51840)
      throw new Error('RSA-1024 names cannot be transferred.');
  ...
  async makeRevoke(name) {
    ...
    if (auction.weak && auction.height < height + 51840)
      throw new Error('RSA-1024 names cannot be revoked.');
