...
class Wallet extends EventEmitter {
  ...
  async makeTransfer(name, hot, cold) {
    assert(typeof name === 'string');
    assert(hot instanceof Address);
    assert(cold instanceof Address);
    ...
    revoke.covenant.items.push(raw);

    const mtx = new MTX();
    mtx.addOutpoint(auction.owner);
    mtx.addOutpoint(auction.revoke);
    mtx.outputs.push(output);
    mtx.outputs.push(revoke);
  ...
  async createTransfer(name, hot, cold, options) {
    const mtx = await this.makeTransfer(name, hot, cold);
    await this.fill(mtx, options);
    return this.finalize(mtx, options);
  }
  ...
  async _sendTransfer(name, hot, cold, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this.createTransfer(name, hot, cold, options);
    return this.sendMTX(mtx, passphrase);
  }
  ...
  async sendTransfer(name, hot, cold, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendTransfer(name, hot, cold, options);
    } finally {
      unlock();
    }
  }
