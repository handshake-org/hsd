...
class Wallet extends EventEmitter {
  ...
  async makeBatchOpen(names, force, acct) {
    ...
    for (const name of names) {
      if (!rules.verifyName(name)) {
        errors.push({name: name, error: `Invalid name: "${name}".`});
        continue;
      }
      ...
      // TODO: Handle expired behavior.
      if (rules.isReserved(nameHash, height, network)) {
        errors.push({name: name, error: `Name is reserved: "${name}".`});
        continue;
      }

      if (!rules.hasRollout(nameHash, height, network)) {
        errors.push({name: name, error: `Name not yet available: "${name}".`});
        continue;
      }
      ...
      if (state !== states.OPENING) {
        errors.push({name: name, error: `Name is not available: "${name}".`});
        continue;
      }

      if (start !== 0 && start !== height) {
        errors.push({name: name, error: `Name is already opening: "${name}".`});
        continue;
      }
