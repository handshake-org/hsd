...
class Wallet extends EventEmitter {
  ...
  async createBid(name, value, lockup, options) {
    ...
    output.address = addr;
    output.value = lockup;
    output.covenant.type = rules.types.BID;
    ...
    return this._finalize(mtx, options);
  }
  ...
  async sendBid(name, value, lockup, options) {
    ...
    return this.__send(mtx, options.passphrase);
  }
  ...
  async createRegister(name, record, options, renew = false) {
    if (!rules.verifyName(name))
      throw new Error('Invalid name.');
    ...
    if (state !== rules.types.REVEAL
        ...
        && state !== rules.types.UPDATE
        && state !== rules.types.TRANSFER) {
      throw new Error('Cannot register (bad state).');
    }

    let type = rules.types.UPDATE;
    ...
    output.covenant.items.push(raw);
    output.covenant.items.push(record.toRaw());

    if (renew) {
      if (state === rules.types.REVEAL)
        throw new Error('Cannot renew.');

      let height = this.wdb.height - rules.RENEWAL_MATURITY * 2;

      if (height < 0)
        height = 0;

      const {hash} = await this.wdb.getBlock(height);
  ...
  async sendRegister(name, record, options) {
    if (!options)
      options = {};
    const mtx = await this.createRegister(name, record, options);
    return this.__send(mtx, options.passphrase);
  }
  ...
  async createRenewal(name, options) {
    ...
    return this.createRegister(name, coin.covenant.items[1], options);
  }
  ...
  async createRedeem(name, options, renew = false) {
    if (!rules.verifyName(name))
      throw new Error('Invalid name.');
    ...
    output.address = coin.address;
    output.value = coin.value;
    output.covenant.type = rules.types.REDEEM;
    output.covenant.items.push(raw);
  ...
  async _send(options, passphrase) {
    ...
    return this.__send(mtx, passphrase);
  }

  async __send(mtx, passphrase) {
