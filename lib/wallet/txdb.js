    ...
    switch (covenant.type) {
      ...
      case types.UPDATE: {
        ...
        if (i + 1 < tx.outputs.length) {
          const output = tx.outputs[i + 1];
          if (output.covenant.type === types.REVOKE)
            revoke = tx.outpoint(i + 1);
        }
...
class Auction {
  ...
  static fromRaw(data) {
    ...
    const revoke = Outpoint.fromReader(br);
    ...
    const data = br.readBytes(br.readU16());
