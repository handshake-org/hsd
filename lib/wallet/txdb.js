...
class TXDB {
  ...
  async erase(wtx, block, medianTime) {
    ...
    if (block) {
      ...
      await this.removeTimeAndCountIndexUnconfirmedUndo(b, hash);
  ...
  async getLatestUnconfirmedTXCount() {
    const raw = await this.bucket.get(layout.I.encode());
    let index = 0;
  ...
  incrementLatestUnconfirmedTXCount(b, count) {
    ...
    b.put(layout.I.encode(), fromU32(count + 1));
  }
  ...
  async getCountForTX(hash) {
    ...
    const raw = await this.bucket.get(layout.y.encode(hash));
  ...
  async getUndoCountForTX(hash) {
    ...
    const raw = await this.bucket.get(layout.x.encode(hash));
  ...
  async addTimeAndCountIndexUnconfirmedUndo(b, hash, time) {
    ...
    b.put(layout.x.encode(hash), count.encode());

    this.incrementLatestUnconfirmedTXCount(b, count.index);
  }

  /**
   * Remove undo unconfirmed time and unconfirmed index.
   * @private
   * @param {Batch} b
   * @param {Hash} hash - Transaction hash.
   * @returns {Promise}
  ...
  async removeTimeAndCountIndexUnconfirmedUndo(b, hash) {
    b.del(layout.e.encode(hash));
    b.del(layout.x.encode(hash));
  }
  ...
  async addCountAndTimeIndexUnconfirmed(b, accounts, hash, time) {
    ...
    for (const [acct] of accounts) {
      b.put(layout.Z.encode(acct, count.height, count.index), hash);
  ...
  async disconnectCountAndTimeIndexUnconfirmed(b, accounts, hash) {
    ...
    b.del(layout.z.encode(height, index));
    b.del(layout.y.encode(hash));

    const time = await this.getUnconfirmedTimeForTX(hash);
    b.del(layout.w.encode(time, index, hash));

    for (const [acct] of accounts) {
      b.del(layout.Z.encode(acct, height, index));
      b.del(layout.W.encode(acct, time, index, hash));
    }
  }
  ...
  async removeCountAndTimeIndexUnconfirmed(b, accounts, hash) {
    ...
    const time = await this.getUnconfirmedTimeForTX(hash);
    b.del(layout.w.encode(time, count.index, hash));
    b.del(layout.e.encode(hash));

    for (const [acct] of accounts) {
      b.del(layout.Z.encode(acct, count.height, count.index));
      b.del(layout.W.encode(acct, time, count.index, hash));
    }
  }
  ...
  async removeCountAndTimeIndex(b, options) {
    ...
    b.del(layout.g.encode(time, count.height,
                          count.index, options.hash));

    for (const [acct] of accounts) {
      b.del(layout.Z.encode(acct, count.height, count.index));

      b.del(layout.G.encode(acct, time, count.height,
                            count.index, options.hash));
    }
  }
  ...
  async listUnconfirmed(acct, options) {
    ...
    if (acct !== -1) {
      hashes = await this.bucket.values({
        gte: layout.Z.min(acct, height),
        lte: layout.Z.max(acct, height),
        limit: options.limit,
        reverse: options.reverse
      });
    } else {
      hashes = await this.bucket.values({
        gte: layout.z.min(height),
        lte: layout.z.max(height),
        limit: options.limit,
        reverse: options.reverse
      });
    }
  ...
  async _listUnconfirmed(acct, options) {
    ...
    if (acct !== -1) {
      if (uopts.reverse) {
        uopts['gte'] = layout.Z.min(acct, height);
        uopts[lesser] = layout.Z.encode(acct, height, index);
      } else {
        uopts[greater] = layout.Z.encode(acct, height, index);
        uopts['lte'] = layout.Z.max(acct, height);
      }
    } else {
      if (uopts.reverse) {
        uopts['gte'] = layout.z.min(height);
        uopts[lesser] = layout.z.encode(height, index);
      } else {
        uopts[greater] = layout.z.encode(height, index);
        uopts['lte'] = layout.z.max(height);
      }
    }
