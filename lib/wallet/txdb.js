...
class TXDB {
  ...
  putBid(b, name, {hash, index}, lockup, blind) {
  ...
  putReveal(b, name, {hash, index}, value, height, own) {
  ...
  unlockBalances(b, state, tx, i, path, height, credits) {
    const {types} = rules;
    const output = tx.outputs[i];
    switch (output.covenant.type) {
      case types.BID: {
      ...
      case types.REVEAL: {
        assert(i < credits.length);
        const {coin} = credits[i];
        assert(coin);
        if (height === -1) {
          state.ulocked(path, coin.value);
          state.ulocked(path, -output.value);
  ...
  async handleCovenant(b, tx, i, path, height) {
    ...
    switch (covenant.type) {
      case types.BID: {
        ...
        if (!auction) {
          this.putAuction(b, name, null, types.BID, height);
          await this.addNameMap(b, name);
        }

        const [, blind] = covenant.items;

        this.putBid(b, name, tx.outpoint(i), output.value, blind);
      ...
      case types.REVEAL: {
        ...
        if (!path) {
          ...
          this.putAuction(b, name, null, types.REVEAL, auction.height);
          await b.write();
          break;
        }
        ...
        const [, nonce] = output.covenant.items;
        const [, blind] = coin.covenant.items;

        this.putBid(b, name, prevout, coin.value, blind);
        this.putBlind(b, blind, output.value, nonce);
        this.putReveal(b, name, tx.outpoint(i), output.value, height, true);
        this.putAuction(b, name, null, types.REVEAL, auction.height);
      ...
      case types.REDEEM: {
        ...
        if (!path) {
          const b = this.bucket.batch();
          this.putAuction(b, name, null, types.REGISTER, auction.height);
          await b.write();
          break;
        }
      ...
      case types.REGISTER: {
        ...
        this.putAuction(b, name, tx.outpoint(i), types.REGISTER, auction.height);
        await this.removeBids(b, name);
        await this.removeReveals(b, name);
      ...
      case types.UPDATE: {
        ...
        this.putAuction(b, name, tx.outpoint(i), types.UPDATE, auction.height);
      ...
      case types.TRANSFER: {
        ...
        this.putAuction(b, name, tx.outpoint(i), types.TRANSFER, auction.height);
      ...
      case types.REVOKE: {
        ...
        this.putAuction(b, name, tx.outpoint(i), types.REVOKE, auction.height);
  ...
  async erase(wtx, block) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      this.unlockBalances(b, state, tx, i, path, height, credits);
