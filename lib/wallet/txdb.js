...
class TXDB {
  ...
  putBid(b, nameHash, outpoint, options) {
    ...
    b.put(layout.i.build(nameHash, hash, index), bb.toRaw());
  }
  ...
  putReveal(b, nameHash, outpoint, options) {
    ...
    brv.own = options.own;
    b.put(layout.B.build(nameHash, hash, index), brv.toRaw());
  }
  ...
  async getWinningReveal(nameHash) {
    const iter = this.bucket.iterator({
      gte: layout.B.min(nameHash),
      lte: layout.B.max(nameHash),
      values: true
  ...
  async isWinningReveal(nameHash, owner) {
    const winner = await this.getWinningReveal(nameHash);
  ...
  async handleCovenant(b, tx, i, path, height) {
    ...
    switch (covenant.type) {
      case types.CLAIM: {
        ...
        this.putAuction(b, nameHash, {
          name: covenant.items[1],
          owner: outpoint,
          state: types.CLAIM,
          height: height,
          data: null
  ...
  async undoCovenant(b, tx, i, path, height) {
    ...
    switch (covenant.type) {
      ...
      case types.BID: {
        ...
        this.removeBid(b, nameHash, outpoint);
      ...
      case types.REVEAL: {
        if (!path) {
          ...
          this.removeReveal(b, nameHash, outpoint);
        ...
        this.removeBid(b, nameHash, tx.inputs[i].prevout);
        this.removeReveal(b, nameHash, outpoint);
...
class Auction {
  ...
  toRaw() {
    const bw = bio.write(1 + this.name.length + 43 + this.data.length);
  ...
  static fromRaw(data) {
    ...
    const name = br.readBytes(br.readU8());
...
class BlindBid {
  ...
  toRaw() {
    const bw = bio.write(1 + this.name.length + 40);
    bw.writeU8(this.name.length);
    bw.writeBytes(this.name);
