...
const AuctionUndo = require('../covenants/undo');
...
class TXDB {
  ...
  async insert(wtx, block) {
    ...
    // Handle auctions.
    if (block) {
      const updated = await this.connectAuctions(b, tx, view, height);
  ...
  async confirm(wtx, block) {
    ...
    // Handle auctions.
    await this.connectAuctions(b, tx, view, height);
  ...
  async connectAuctions(b, tx, view, height) {
    const {types} = rules;
    const hash = tx.hash('hex');
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      switch (covenant.type) {
        case types.CLAIM: {
          ...
          const h = await this.wdb.getOpeningHeight(nameHash, height);

          auction.setAuction(name, h);
    ...
    for (const auction of view.auctions.values()) {
      ...
      auction.applyState(auction.ops);
