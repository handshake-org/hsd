...
class TXDB {
  ...
  addNameMap(b, name) {
    return this.wdb.addNameMap(b.root(), name, this.wid);
  }
  ...
  removeNameMap(b, name) {
    return this.wdb.removeNameMap(b.root(), name, this.wid);
  }
  ...
  /**
   * Test whether we have an auction.
   * @param {String} name
   * @returns {Boolean}
   */

  async hasAuction(name) {
    return this.bucket.has(layout.A.build(name));
  }

  /**
   * Get an auction if present.
   * @param {String|Buffer} name
   * @returns {Auction}
   */

  async getAuction(name) {
    const raw = await this.bucket.get(layout.A.build(name));

    if (!raw)
      return null;

    const auction = Auction.fromRaw(raw);
    auction.name = toString(name);
  ...
  putAuction(b, name, options) {
    let {owner, cold, data} = options;
    ...
    auction.owner = owner;
    auction.cold = cold;
    auction.state = options.state;
    auction.height = options.height;
    auction.data = data;

    b.put(layout.A.build(name), auction.toRaw());
  }
  ...
  putBid(b, name, outpoint, options) {
    const {hash, index} = outpoint;
    const bb = new BlindBid();
    bb.lockup = options.lockup;
    bb.blind = options.blind;
    b.put(layout.i.build(name, hash, index), bb.toRaw());
  }
  ...
  removeBid(b, name, outpoint) {
    const {hash, index} = outpoint;
    b.del(layout.i.build(name, hash, index));
  }
  ...
  async hasReveal(name, outpoint) {
    const {hash, index} = outpoint;
    return this.bucket.has(layout.B.build(name, hash, index));
  }
  ...
  async getReveal(name, outpoint) {
    const {hash, index} = outpoint;
    const raw = await this.bucket.get(layout.B.build(name, hash, index));

    if (!raw)
      return null;

    const brv = BidReveal.fromRaw(raw);
    brv.name = toString(name);
  ...
  putReveal(b, name, outpoint, options) {
    const {hash, index} = outpoint;
    const brv = new BidReveal();
    brv.value = options.value;
    brv.height = options.height;
    brv.own = options.own;
    b.put(layout.B.build(name, hash, index), brv.toRaw());
  }
  ...
  removeReveal(b, name, outpoint) {
    const {hash, index} = outpoint;
    b.del(layout.B.build(name, hash, index));
  }
  ...
  /**
   * Get the winning reveal for an auction.
   * @param {String|Buffer} name
   * @returns {Outpoint}
   * @throws on winner not found
   */
  ...
  async getWinningReveal(name) {
    ...
    await iter.each((key, data) => {
      const val = bio.read(data).readU64();
  ...
  /**
   * Test whether an outpoint is a winner.
   * @param {String} name
   * @param {Outpoint} owner
   * @returns {Boolean}
   */
  ...
  putBlind(b, blind, options) {
    ...
    bv.value = value;
    bv.nonce = nonce;
    b.put(layout.v.build(blind), bv.toRaw());
  }
  ...
  async insert(wtx, block) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      await this.handleCovenant(b, tx, i, path, height);
  ...
  async lockBalances(b, state, tx, i, path, height) {
    ...
    switch (covenant.type) {
      ...
      case types.REVEAL: {
        ...
        const bb = await this.getBid(name, prevout);
        assert(bb);
  ...
  async handleCovenant(b, tx, i, path, height) {
    const {types} = rules;
    const output = tx.outputs[i];
    ...
    switch (covenant.type) {
      ...
      case types.BID: {
        ...
        this.putBid(b, name, outpoint, {
          lockup: output.value,
          blind: covenant.items[1]
        });
      ...
      case types.REVEAL: {
        ...
        if (!path) {
          ...
          this.putReveal(b, name, outpoint, {
            value: output.value,
            height: height,
            own: false
          });
        ...
        // this.putBlind(b, uc.items[1], {
        //   value: output.value,
        //   nonce: covenant.items[1]
        // });
  ...
  async undoCovenant(b, tx, i, path, height) {
    const {types} = rules;
    const output = tx.outputs[i];
    const {covenant} = output;
    ...
    switch (covenant.type) {
      ...
      case types.BID: {
        ...
        this.removeBid(b, name, outpoint);
      ...
      case types.REVEAL: {
        if (!path) {
          ...
          this.removeReveal(b, name, outpoint);
        ...
        this.removeBid(b, name, tx.inputs[i].prevout);
        this.removeReveal(b, name, outpoint);
...
class Auction {
  ...
  toRaw() {
    const bw = bio.write(79 + this.data.length);
    ...
    this.cold.toWriter(bw);
    bw.writeU8(this.state);
    bw.writeU32(height);
  ...
  static fromRaw(data) {
    ...
    const cold = Outpoint.fromReader(br);
