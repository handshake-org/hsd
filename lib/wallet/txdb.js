...
class TXDB {
  ...
  async recalculateBalances() {
    ...
    for (const [acct, delta] of state.accounts) {
      const oldAcctBalanceRaw = await this.bucket.get(layout.r.encode(acct));
      const oldAcctBalance = Balance.decode(oldAcctBalanceRaw);
      const finalAcctBalance = new Balance();
      finalAcctBalance.tx = oldAcctBalance.tx;

      delta.applyTo(finalAcctBalance);
      batch.put(layout.r.encode(acct), finalAcctBalance.encode());
    }

    const rawWalletBalance = await this.bucket.get(layout.R.encode());
    const walletBalance = Balance.decode(rawWalletBalance);
