...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add("getaccountbyaddress", this.getAccountByAddress);
    ...
    this.add("sendrevealforaccount", this.sendRevealForAccount);
  ...
  async getAccountByAddress(args, help) {
    if (help || args.length !== 1) {
      throw new RPCError(errs.MISC_ERROR, 'getaccountbyaddress "address"');
    }
    ...
    const account = await wallet.getAccountByAddress(new Address(addr));
  ...
  async sendBid(args, help) {
    ...
    const tx = await wallet.sendBid(name, bid, value, { account });
  ...
  async sendRevealForAccount(args, help) {
    if (help || args.length !== 2) {
      throw new RPCError(errs.MISC_ERROR, 'sendrevealforaccount "name"');
    }
    ...
    if (!account) {
      throw new RPCError(errs.TYPE_ERROR, "Invalid account.");
    }
    ...
    const tx = await wallet.sendRevealForAccount(name, account);
  ...
  async sendUpdate(args, help) {
    if (help || args.length !== 3) {
      throw new RPCError(
        errs.MISC_ERROR,
        'sendupdate "name" "data" "accountIndex"'
      );
    }
    const wallet = this.wallet;
    const valid = new Validator(args);
    ...
    const tx = await wallet.sendUpdate(name, resource, { account });
