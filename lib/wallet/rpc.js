...
class RPC extends RPCBase {
  ...
  async _toWalletTX(wtx) {
    ...
    return {
      ...
      blockhash: details.block ? details.block : null,
  ...
  async importAddress(args, help) {
    ...
    if (p2sh) {
      ...
      addr = Address.fromScripthash(script.blake256());
    } else {
  ...
  async getAuctions(args, help) {
    ...
    const auctions = await wallet.getAuctions(name);
    const items = [];

    for (const auction of auctions)
      items.push(auction.toJSON());
  ...
  async sendUpdate(args, help) {
    ...
    const resource = HSKResource.fromJSON(data);
  ...
  async sendTransfer(args, help) {
    if (help || args.length !== 2)
      throw new RPCError(errs.MISC_ERROR, 'sendtransfer "name" "hot" "cold"');

    const wallet = this.wallet;
    const valid = new Validator(args);
    const name = valid.str(0);
    const addr1 = valid.str(1);
    const addr2 = valid.str(2);
    ...
    const hot = parseAddress(addr1, this.network);
    const cold = parseAddress(addr2, this.network);

    const tx = await wallet.sendTransfer(name, hot, cold);
