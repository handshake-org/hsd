...
class RPC extends RPCBase {
  ...
  async sendOpen(args, help) {
    if (help || args.length < 1 || args.length > 3)
      throw new RPCError(errs.MISC_ERROR, 'sendopen "name" ( force ) ( account )');

    const wallet = this.wallet;
    const valid = new Validator(args);
    const name = valid.str(0);
    const force = valid.bool(1, false);
    let account = valid.str(2, '');
  ...
  async sendReveal(args, help) {
    if (help || args.length > 2)
      throw new RPCError(errs.MISC_ERROR, 'sendreveal "name" ( account )');

    const wallet = this.wallet;
    const valid = new Validator(args);
    const name = valid.str(0);
    let account = valid.str(0, '');
