...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('sendrawupdateidempotent', this.sendRawUpdateIdempotent);
  ...
  async sendUpdateIdempotent(args, help) {
    const opts = this._validateUpdateItempotent(args, help, 'sendupdateidempotent');
    const wallet = this.wallet;
    const tx = await wallet.sendUpdate(opts.name, opts.update, opts.value, {
      account: opts.account,
      idempotencyKey: opts.idempotencyKey,
    });
  ...
  _validateUpdateItempotent(args, help, method) {
    ...
    const valid = new Validator(args);
    const name = valid.str(0);
    const update = valid.ufixed(1, EXP);
    const value = valid.ufixed(2, EXP);
    const idempotencyKey = valid.str(3);
    const account = valid.str(4);
  ...
  async sendRawUpdateIdempotent(args, help) {
    const opts = this._validateRawUpdateItempotent(args, help, 'sendrawupdateidempotent');
    const wallet = this.wallet;
    const tx = await wallet.sendUpdate(opts.name, opts.update, opts.value, {
      account: opts.account,
      idempotencyKey: opts.idempotencyKey,
