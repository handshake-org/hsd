...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('derivereceive', this.deriveReceive);
  ...
  async deriveReceive(args, help) {
    if (help || args.length > 2)
      throw new RPCError(errs.MISC_ERROR, 'derivereceive ( "account", "index")');
    ...
    const index = valid.u32(1, 0);
    return wallet.deriveReceive(name, index)
  }
