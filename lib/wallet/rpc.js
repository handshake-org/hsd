...
class RPC extends RPCBase {
  ...
  async getAuctions(args, help) {
    ...
    for (const auction of auctions)
      items.push(auction.getJSON(height, network));
  ...
  async getAuctionInfo(args, help) {
    ...
    const auction = await wallet.getAuctionByName(name);
    ...
    const info = auction.getJSON(height, network);
    info.bids = [];
    info.reveals = [];
  ...
  async getNameInfo(args, help) {
    ...
    return auction.getJSON(height, network);
  }
  ...
  async getNameByHash(args, help) {
    ...
    const auction = await wallet.getAuction(hash);
  ...
  async sendClaim(args, help) {
    if (help || args.length !== 1)
      throw new RPCError(errs.MISC_ERROR, 'sendbid "name"');
    ...
    const tx = await wallet.sendClaim(name);
