...
class WalletDB extends EventEmitter {
  ...
  addName(name) {
    this.filter.add(name, 'ascii');
    return this.addFilter(name);
  }
  ...
  async getWalletsByTX(tx) {
    ...
    for (const {covenant} of tx.outputs) {
      ...
      if (!this.testFilter(covenant.items[0]))
        continue;

      const map = await this.getNameMap(covenant.items[0]);
  ...
  async getNameMap(name) {
    return this.getMap(layout.N.build(name));
  }
  ...
  async addNameMap(b, name, wid) {
    await this.addName(name);
    return this.addMap(b, layout.N.build(name), wid);
  }
  ...
  async removeNameMap(b, name, wid) {
    return this.removeMap(b, layout.N.build(name), wid);
  }
  ...
  async getRenewalBlock() {
    let height = this.height - rules.RENEWAL_MATURITY * 2;
