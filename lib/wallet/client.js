...
function parseEntry(data) {
  ...
  assert(data.length >= size + 9);

  const size = consensus.HEADER_SIZE;
  const len = size + 1 + data[size] * 4;

  const h = blake2b.digest(data.slice(0, len));

  return {
    hash: h.toString('hex'),
    height: data.readUInt32LE(len, true),
    time: data.readUInt32LE(132, true)
  };
}
