...
const Escher = require('../covenants/escher');
...
class Chain extends AsyncEmitter {
  ...
  async syncTree() {
    ...
    for (let height = commitHeight + 1; height <= this.height; height++) {
      ...
      const escher = state.hasEscher();

      for (const tx of block.txs)
        await this.verifyCovenants(tx, view, height, hardened, escher);
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.hasEscher() && state.hasEscher())
      this.logger.warning('Escher update chains have been activated.');
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      switch (covenant.type) {
        ...
        case types.UPDATE: {
          ...
          if (escher && !Escher.verify(data, ns.data)) {
            throw new VerifyError(tx,
              'invalid',
              'bad-update-escher',
              100);
