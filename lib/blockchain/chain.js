...
class Chain extends AsyncEmitter {
  ...
  async verifyInputs(block, prev, state) {
    ...
    if (block.treeRoot !== this.db.treeRoot()) {
      throw new VerifyError(block,
        'invalid',
        'bad-tree-root',
        100);
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      if (!await this.cdb.process(tx, view, height)) {
        throw new VerifyError(block,
          'invalid',
          'invalid-covenant',
          100);
  ...
  async retarget(last) {
    ...
    //      to negative numbers!
    //   3. Zcash's implementation has a bug where
    //      the average will overflow on numbers
    //      greater than 256 during the averaging.
