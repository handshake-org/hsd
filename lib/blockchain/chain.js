...
class Chain extends AsyncEmitter {
  ...
  /**
   * Check for compromised names (soft-fork activated).
   * @param {NameState} ns
   * @param {Number} height
  ...
  async isCompromisedName(ns, height, hardened) {
    assert(ns instanceof NameState);
    assert((height >>> 0) === height);
    assert(typeof hardened === 'boolean');

    // Must be a name which was claimed previously.
    if (!ns.claimed)
      return false;

    // If it's registered, that means
    // the coin reward has been redeemed.
    // To prevent a massive inflationary
    // event, we can't help these names.
    // Hopefully miners will come to
    // consensus within 1 month.
    if (ns.registered)
      return false;

    // The claim window must still be open.
    // These names shouldn't be locked forever!
    if (!ns.isClaimable(height, this.network))
      return false;
  ...
  async verifyCovenants(tx, view, height, hardened) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      let state = ns.state(height, network);

      // If the name needs hardening, it
      // becomes locked until the reservation
      // period ends. Note that this is the
      // same code path that can be used for
      // emergency soft-forks in the case
      // that a large name registrar's keys
      // are compromised.
      if (await this.isCompromisedName(ns, height, hardened)) {
        assert(state === states.LOCKED
            || state === states.CLOSED);
        state = states.LOCKED;
      }
      ...
      if (covenant.isClaim()) {
        ...
        // Note that a newer claim invalidates
        // the old output by committing to a
        // higher height.
        if (state > states.LOCKED) {
          // Claims can also be re-redeemed any
          // time before registration. This is
          // required for our emergency soft-forks
          // to truly behave as _soft_ forks.
          if (state !== states.CLOSED || ns.registered) {
        ...
        // In addition to the isCompromisedName
        // call, any future emergency soft-forks
        // should also be included below this
        // check.
        if (hardened && weak) {
          throw new VerifyError(tx,
            'invalid',
        ...
        // Check owner status to allow for the genesis block.
        if (!ns.owner.isNull() && claimed <= ns.claimed) {
          throw new VerifyError(tx,
            'invalid',
  ...
  async retarget(last) {
    ...
    // Note that `x >> 2` rounds towards negative infinity, NOT zero.
    // Math.floor(x / 4) would also round towards negative infinity.
    // True zero-rounding integer division would be achieved with:
    //   y = x / 4;
    //   if (y < 0)
    //     return -Math.floor(-y);
    //   return y;
