...
class Chain extends AsyncEmitter {
  ...
  async verify(block, prev, flags) {
    ...
    // Ensure we have no more than 500 name updates.
    let updates = 0;
  ...
  async updateInputs(block, prev) {
    ...
    for (let i = 1; i < block.txs.length; i++) {
      ...
      if (!await this.cdb.process(tx, view, height, false)) {
        throw new VerifyError(block,
          'invalid',
          'invalid-covenant',
          100);
  ...
  async verifyInputs(block, prev, state) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      if (!await this.cdb.process(tx, view, height, hardened)) {
        throw new VerifyError(block,
          'invalid',
          'invalid-covenant',
          100);
  ...
  async _add(block, flags, id) {
    ...
    if (this.hasOrphan(hash)) {
      this.logger.debug('Already have orphan block: %s.', block.hash());
      throw new VerifyError(block, 'duplicate', 'duplicate', 0);
    }

    // Do not revalidate known invalid blocks.
    if (this.hasInvalid(block)) {
      this.logger.debug('Invalid ancestors for block: %s.', block.hash());
    ...
    if (await this.hasEntry(hash)) {
      this.logger.debug('Already have block: %s.', block.hash());
      throw new VerifyError(block, 'duplicate', 'duplicate', 0);
    }
  ...
  maybeSync() {
    ...
    if (now < this.network.block.launchDate) {
      this.synced = true;
      this.emit('full');
      return;
    }
    ...
    if (this.tip.time < now - this.network.block.maxTipAge)
      return;
  ...
  async retarget(last) {
    ...
    this.logger.warning('Retargetting to: %s (0x%s).',
