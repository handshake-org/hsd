...
const Tree = require('btrie/research/merklix/merklix');
...
const CDB = require('../covenants/db');
...
class ChainDB {
  ...
  constructor(options) {
    ...
    const prefix = !this.options.memory
      ? this.options.treePrefix
      : null;

    this.db = bdb.create(this.options);
    this.treeBucket = this.db.bucket(layout.k.build());
    this.tree = new Tree(blake2b, 160, prefix, null, 0);
    this.cdb = new CDB(this);
  ...
  async open() {
    ...
    this.logger.info(
      'Chain State: hash=%s tx=%d coin=%d value=%s burned=%s.',
      this.state.tip,
      this.state.tx,
      this.state.coin,
    ...
    this.logger.info('Tree Root: %s.', this.tree.rootHash('hex'));
  }
  ...
  start() {
    ...
    this.snapshot = this.tree.snapshotSync();
  ...
  async lookup(root, key) {
    const tree = await this.tree.snapshot(root);
    return tree.get(key);
  }
  ...
  async saveGenesis() {
    ...
    const block = Block.fromRaw(network.genesisBlock);
    ...
    for (let i = 1; i < block.txs.length; i++) {
      ...
      assert(await this.cdb.process(tx, view, 0));
    }
  ...
  async scan(start, filter, iter) {
    ...
    if (typeof start === 'number')
      ...
      this.logger.info('Scanning from block %s.', start);
  ...
  async reset(block) {
    ...
    this.logger.debug('Resetting main chain to: %s', entry.hash);
  ...
  async _removeChain(hash) {
    ...
    this.logger.debug('Removing alternate chain: %s.', tip.hash);
  ...
  /**
   * Commit the current name tree snapshot.
   */

  async commitSnapshot() {
    const b = this.treeBucket.wrap(this.current);

    await this.snapshot.commit(b);

    this.put(layout.s.build(), this.snapshot.rootHash());
  }
  ...
  async connectBlock(entry, block, view) {
    ...
    // Update any contracts.
    await this.cdb.connect(this.current, this.snapshot, view, entry.height);

    // Write the snapshotted tree.
  ...
  async disconnectBlock(entry, block) {
    ...
    // Undo any contracts.
    await this.cdb.disconnect(this.current, this.snapshot, view, entry.height);

    // Write the snapshotted name tree.
    await this.commitSnapshot();
