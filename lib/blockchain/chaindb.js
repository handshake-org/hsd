...
class ChainDB {
  ...
  async getAuctionStatus(nameHash, height, hardened) {
    if (hardened == null)
      hardened = false;
  ...
  async saveAuctions(view, height) {
    for (const auction of view.auctions.values()) {
      ...
      await this.txn.insert(nameHash, auction.encode());
    }
  ...
  async connectAuctions(view, height) {
    ...
    return this.saveAuctions(view, height);
  }

  async disconnectAuctions(view, height) {
    ...
    return this.saveAuctions(view, height - 1);
  }
  ...
  async connectBlock(entry, block, view) {
    ...
    // Connect auction state.
    return this.connectAuctions(view, entry.height);
  }
  ...
  async disconnectBlock(entry, block) {
    ...
    // Connect auction state.
    await this.disconnectAuctions(view, entry.height);
