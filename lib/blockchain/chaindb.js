...
class ChainDB {
  ...
  async saveView(view, height) {
    ...
    return this.cdb.saveView(this.current, view, height);
  }
  ...
  async connectBlock(entry, block, view) {
    ...
    await this.saveView(view, entry.height);
  ...
  async disconnectBlock(entry, block) {
    ...
    // Undo any contracts.
    await this.cdb.revert(this.current, view, entry.height);
