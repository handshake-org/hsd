...
class ChainDB {
  ...
  async connectBlock(entry, block, view) {
    ...
    // Start an urkel transaction.
    const txn = this.tree.txn();

    // Update any contracts.
    await this.cdb.connect(this.current, txn, view, entry.height);

    // Write to the name tree.
    this.put(layout.s.build(), await txn.commit());
  ...
  async disconnectBlock(entry, block) {
    ...
    // Undo any contracts.
    await this.cdb.disconnect(this.current, txn, view, entry.height);
