...
class Mempool extends EventEmitter {
  ...
  async open() {
    ...
    if (this.options.persistent) {
      ...
      const escher = await this.hasEscher();
      ...
      for (const entry of entries) {
        ...
        assert(await this.verifyCovenants(
          entry.tx,
          view,
          height,
          hardened,
  ...
  /**
   * Test whether Escher update chains are enforced.
   * @returns {Boolean}
   */

  async hasEscher() {
    const state = await this.getNextState();
    return state.hasEscher();
  }
  ...
  async insertClaim(claim, id) {
    ...
    if (!await this.verifyCovenants(tx, view, height, hardened, escher)) {
   ...
   * @param {Boolean} escher
   */

  async verifyCovenants(tx, view, height, hardened, escher) {
    try {
      await this.chain.verifyCovenants(tx, view, height, hardened, escher);
    } catch (e) {
