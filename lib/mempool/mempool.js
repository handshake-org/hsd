...
class Mempool extends EventEmitter {
  ...
  async open() {
    ...
    if (this.options.persistent) {
      ...
      for (const entry of entries) {
        ...
        await this.chain.cdb.process(entry.tx, view, height, hardened);

        this.auctions.track(entry.tx, view);
      }
  ...
  async _addBlock(block, txs, view) {
    if (this.map.size === 0 && this.claims.size === 0) {
      this.tip = block.hash;
      return;
    }
    ...
    for (const {covenant} of cb.outputs) {
    ...
    // our watched auction states.
    this.auctions.merge(view);

    // Invalidate any auction-modifying
    // transactions in our mempool. This
    // could be, for example, an UPDATE
    // for a now expired auction. Another
    // example is a stale BID for an auction
    // which has now reached the REVEAL state.
    const invalid = this.auctions.invalidate(block.height, false);
  ...
  async _handleReorg() {
    ...
    for (const [hash, entry] of this.map) {
      ...
      if (hasCovenants) {
        ...
        if (!await this.chain.cdb.process(entry.tx, view, height, hardened)) {
          remove.push(hash);
          continue;
        }
  ...
  async getNextState() {
    if (this.nextState.tip !== this.chain.tip.hash)
  ...
  /**
   * Test whether the chain state
   * supports the ksk-2017 rollover.
   * @returns {Boolean}
   */

  async hasRollover() {
    const state = await this.getNextState();
    return state.hasRollover();
  }
  ...
  limitSize(added) {
    ...
    if (this.size <= threshold)
      return !(this.hasEntry(added) || this.hasClaim(added));
  ...
  async insertClaim(claim, id) {
    ...
    if (this.claimNames.has(nameHex)) {
      throw new VerifyError(claim,
        'alreadyknown',
        'txn-already-in-mempool',
        0);
    ...
    if (this.auctions.names.has(nameHex)) {
      throw new VerifyError(claim,
        'alreadyknown',
        'txn-already-in-mempool',
        0);
    ...
    if (!data.rollover) {
      if (await this.hasRollover())
        throw new VerifyError(claim, 'invalid', 'bad-claim-rollover', 0);
    }

    const tx = claim.toTX(this.network);
    const view = new CoinView();

    // Covenant verification.
    if (!await this.chain.cdb.process(tx, view, height, hardened)) {
  ...
  /**
   * Test whether a TX performs an
   * operation on a to-be-claimed name.
   * @returns {Boolean}
   */
  ...
  hasOutputClaim(tx) {
    for (const {covenant} of tx.outputs) {
      ...
      if (this.claimNames.has(nameHash))
        return true;
    }
  ...
  /**
   * Test whether a TX has a duplicate opening name.
   * @returns {Boolean}
   */
  ...
  hasDuplicateOpen(tx) {
    for (const {covenant} of tx.outputs) {
      ...
      if (this.auctions.opens.has(nameHash))
        return true;
    }
  ...
  trackClaim(entry, id) {
    ...
    this.logger.debug(
      'Added claim %s to mempool (name=%s, claims=%d).',
      entry.hash, entry.nameHash, this.claims.size);
  }
  ...
  async insertTX(tx, id) {
    ...
    // We shouldn't allow a TX which performs
    // an operation on an existing claim.
    if (this.hasOutputClaim(tx)) {
      throw new VerifyError(tx,
        'alreadyknown',
        'txn-already-in-mempool',
        0);
    ...
    // No double opens for a name.
    if (this.hasDuplicateOpen(tx)) {
      throw new VerifyError(tx,
        'alreadyknown',
        'txn-already-in-mempool',
        0);
  ...
  limitOrphans() {
    ...
    this.logger.debug('Removing orphan %s from mempool.', hash);
  ...
  removeDoubleOpens(tx) {
    ...
    for (const {covenant} of tx.outputs) {
      ...
      const nameHash = covenant.items[0].toString('hex');
      const set = this.auctions.opens.get(nameHash);
