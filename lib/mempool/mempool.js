...
const ContractState = require('./auctionstate');
...
class Mempool extends EventEmitter {
  ...
  async _addBlock(block, txs, view) {
    ...
    const invalid = this.contracts.invalidate(block.height, false);
  ...
  async insertClaim(claim, id) {
    ...
    if (this.contracts.refs.has(nameHash)) {
      throw new VerifyError(claim,
        'alreadyknown',
        'txn-already-in-mempool',
        0);
  ...
  hasDuplicateOpen(tx) {
    for (const {covenant} of tx.outputs) {
      ...
      if (this.contracts.opens.has(nameHash))
        return true;
    }
