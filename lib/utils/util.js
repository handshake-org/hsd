 ...
 *
 * @param {ProcessedOutput} processedOutput asd
 * @param {LRU} cache asd
 ...
 *
 * @param {ProcessedOutput} processedOutput 
 * @param {LRU} cache processedOutputCache
 * @returns {undefined}
 */
util.storeMultipleInCache = function (processedOutput, cache) {
  this.storeInCache(processedOutput, cache, (cache, processedOutput) => {
    const idempotencyKey = processedOutput.idempotency_key;
    if (cache.has(idempotencyKey)) {
      cache.get(idempotencyKey).push(processedOutput);
    } else {
      cache.set(idempotencyKey, [processedOutput]);
    }
  });
};
