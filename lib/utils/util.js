...
/**
 * Create a valid mtx with a limit
 * number of outputs
 * @param {limit} number
 * @param {domainOutputMap} Map<string, Array<T>>
 * @returns { validDomains: Array<string>, invalidDomains: Array<string> }
 */
...
util.createBatch = function(limit, domainOutputMap) {
  ...
  const entriesArr = [...domainOutputMap.entries()];
  entriesArr.sort((elem1, elem2) => elem1[1].length > elem2[1].length);

  const rejectedDomains = [];
  const validDomains = [];
  let totalNumberOfOutputs = 0;
  for (const [name, outputs] of entriesArr) {
      const outputLen = totalNumberOfOutputs + outputs.length;
      if (outputLen > limit) {
          rejectedDomains.push(name);
      } else {
          totalNumberOfOutputs += outputs.length;
          validDomains.push(name);
      }
  }
