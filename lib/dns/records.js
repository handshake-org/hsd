...
class Service extends Struct {
  ...
  read(br, d) {
    this.service = d.readString(br, 32);
    this.protocol = d.readString(br, 32);
...
class Magnet extends Struct {
  ...
  read(br, d) {
    this.nid = d.readString(br, 32);
    const size = br.readU8();
    assert(size <= 64);
    this.nin = br.readString('hex', size);
    return this;
  }
...
class SMIME extends Struct {
  ...
  fromJSON(json) {
    ...
    assert(typeof json.fingerprint === 'string');
    assert((json.fingerprint.length >>> 1) <= 255);
    this.hash = Buffer.from(json.hash, 'hex');
    assert(this.hash.length === (json.hash.length >>> 1));
    this.port = json.port;
    this.usage = json.usage;
    this.selector = json.selector;
    this.matchingType = json.matchingType;
...
class PGP extends Struct {
  ...
  getSize() {
    return 2 + this.publicKey.length;
  }

  write(bw) {
    bw.writeU16(this.publicKey.length);
  ...
  fromJSON(json) {
    ...
    assert(this.publicKey.length === (json.publicKey.length >>> 1));
    return this;
  }
}
...
class Addr extends Struct {
  ...
  read(br, d) {
    ...
    this.currency = d.readString(br, 32);

    const size = br.readU8();
    assert(size <= 255);

    this.address = readAscii(br, size);
