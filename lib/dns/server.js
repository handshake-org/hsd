...
class RootServer extends DNSServer {
  ...
  async lookupName(name) {
    ...
    const hash = rules.hashName(name.toLowerCase());
  ...
  async response(req, rinfo) {
    ...
    const data = await this.lookupName(name);
...
class RecursiveServer extends DNSServer {
  constructor(options) {
    ...
    this.icann = new UnboundResolver({
      inet6: false,
      tcp: true,
      edns: true,
      dnssec: true,
  ...
  async close() {
    await super.close();
    await this.icann.close();
    await this.hns.close();
  }

  async resolve(req, rinfo) {
    const [qs] = req.question;
    const res = await this.hns.resolve(qs);

    if (!isRootNX(res))
      return res;

    const name = util.label(qs.name, -1).toLowerCase();
    const item = reserved.get(name);

    if (!item || !item.root)
      return res;

    return this.icann.resolve(qs);
  }
}
...
function isRootNX(res) {
  if (res.code !== codes.NXDOMAIN)
    return false;

  const soa = util.extractSet(res.authority, '.', types.SOA);
