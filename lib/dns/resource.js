...
class Resource extends Struct {
  ...
  toDNS(name, type) {
    ...
    if (labels.length === 3) {
      switch (type) {
        case types.SRV: {
          const [srv, protocol, service] = decodeSRV(name, labels);
          if (srv) {
        ...
        case types.TLSA: {
          const [tlsa, protocol, port] = decodeTLSA(name);
          if (tlsa) {
        ...
        case types.SMIMEA: {
          const [smimea, hash] = decodeSMIMEA(name);
          if (smimea) {
...
function decodeSRV(name, labels) {
  ...
  return [true, `${protocol.substring(1)}.`, `${service.substring(1)}.`];
}
...
function isSRV(name) {
  ...
  return decodeSRV(name, labels)[0];
}
