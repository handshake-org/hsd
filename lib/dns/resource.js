...
class Resource extends Struct {
  ...
  toNSEC(name, map) {
    return createNSEC(name, 86400, map);
  }
...
function createNSEC(name, ttl, map) {
  assert(util.isFQDN(name));
  assert((ttl >>> 0) === ttl);
  assert(!map || Buffer.isBuffer(map));
